...
- The experiment runner writes a simple `metrics.csv` file to the `log_dir` (e.g., `results/sac_experiment/metrics.csv`) with columns `step,avg_reward,std_reward` for each evaluation step.
- Use `notebooks/experiment_template.ipynb` to load and visualize `metrics.csv` (the notebook includes example plotting and smoothing cells).

Example Python snippet (run in the notebook):

```python
import pandas as pd
import matplotlib.pyplot as plt

df = pd.read_csv('results/sac_experiment/metrics.csv')
plt.plot(df['step'], df['avg_reward'])
plt.fill_between(df['step'], df['avg_reward'] - df['std_reward'], df['avg_reward'] + df['std_reward'], alpha=0.2)
...
- A lightweight GitHub Actions workflow is included at `.github/workflows/ci.yml` to run fast unit tests on pushes and pull requests. The CI installs the minimal dependencies and runs `pytest` in `paperAssignments/Assignments1-50/CA29`.
- A slower/longer smoke test `tests/test_smoke.py` exists but is **skipped by default** (disabled unless `RUN_SMOKE=1`). Run locally with:

```bash
# Run the fast test suite
pytest -q

# Run the optional smoke test (slower):
RUN_SMOKE=1 pytest -q
```

This allows CI to keep quick feedback while giving you a way to run more thorough sanity checks locally or in a separate job.
