...
The combination of reward transformations and distributional losses is underexplored. Reward shaping can densify returns but may introduce bias or variance in distributional estimates. Conversely, distributional methods can capture shaping-induced variance but may amplify noise in sparse settings. We hypothesize that the To-the-Max transform, which provides progress-based bonuses, synergizes well with Sinkhorn losses by reducing distributional variance and enabling smaller regularization blur.
...
\textbf{Reward shaping and potential methods.} Potential-based shaping \cite{ng1999policy} ensures policy invariance and remains a common theoretical tool. Empirical works have explored learned shaping and relabeling approaches such as HER \cite{andrychowicz2017hindsight} to accelerate sparse-goal learning in robotics. More recent approaches include learned reward shaping via auxiliary networks \cite{harutyunyan2019hindsight} and curriculum-based shaping for hierarchical tasks \cite{florensa2017reverse}. Our To-the-Max transform builds on these by providing a simple, interpretable progress-based bonus that can be made potential-based for theoretical guarantees.

\textbf{Distributional RL.} C51 \cite{bellemare2017distributional} and QR-DQN \cite{dabney2018distributional} established distributional perspectives; particle-based critics and implicit quantile networks extend these ideas to richer, continuous outputs. Recent advances include Stein distributional methods \cite{rowland2019stein} and neural variants for high-dimensional spaces \cite{yang2019fully}. Sinkhorn-based distributional critics \cite{abdullah2020wasserstein} have shown promise in stabilizing training compared to raw Wasserstein losses.

\textbf{Optimal transport in ML.} Entropic OT and Sinkhorn divergences are used in generative modeling and matching problems due to their stable gradients and scalability (Cuturi \cite{cuturi2013sinkhorn}, Feydy et al. \cite{feydy2019interpolating}). In RL, OT has been applied to policy optimization \cite{metz2017discrete} and value function approximation \cite{abdullah2020wasserstein}. Our use of Sinkhorn divergence for distributional critics follows this trend but integrates it with reward transformations.

\textbf{Intersection of shaping and distributional learning.} Prior work typically treats shaping or distributional losses separately; our work studies their interaction empirically and theoretically. While some papers combine shaping with expectation-based critics \cite{devlin2012dynamic}, the synergy with distributional methods remains underexplored. We hypothesize that shaping reduces distributional variance, enabling more stable OT computations.
