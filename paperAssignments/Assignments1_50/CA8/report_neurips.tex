...
Reward shaping and potential-based methods. Potential-based shaping \cite{ng1999policy} guarantees policy invariance and remains a foundational result for shaping literature. Recent work has explored learned and task-specific shaping signals (heuristics or learned progress estimators) to accelerate sparse-goal learning (e.g., hindsight experience replay (HER) \cite{andrychowicz2017hindsight} combined with shaping in robotics contexts).

Distributional RL. Distributional approaches (C51 \cite{bellemare2017distributional}, QR-DQN \cite{dabney2018distributional}, implicit quantile networks, and particle-based methods) show improved stability and richer value estimates; these have been applied to both discrete and continuous control. Particle-based distributional critics capture multimodality which is useful when multiple strategies exist for achieving goals.
