...
Sparse-reward goal-reaching remains one of the most challenging settings for modern reinforcement learning algorithms: agents receive informative feedback only rarely, which leads to long episodes of unguided exploration and noisy gradient estimates. Although many approaches exist to address sparse rewards (e.g., exploration bonuses, intrinsic motivation, curriculum learning), designing methods that are broadly applicable and theoretically grounded remains an open problem. 

Reward shaping is a classic and practical tool that can densify learning signals, but careless shaping can introduce bias that changes optimal policies unless the shaping is potential-based \cite{ng1999policy}. Recent empirical work shows that appropriate shaping can dramatically accelerate learning in goal-conditioned tasks, but the interaction between shaping and modern loss functions (e.g., distributional OT losses) has not been systematically explored.

Distributional reinforcement learning models return distributions instead of only expected returns \cite{bellemare2017distributional,dabney2018distributional}. Particle-based distributional critics can represent multimodal returns that arise in stochastic or multi-path domains. Sinkhorn divergences, an entropically regularized form of optimal transport, provide geometry-aware loss functions with smoother gradients than direct Wasserstein distances \cite{feydy2019interpolating,genevay2018learning}. We hypothesize that reward transformations that compress or concentrate returns may interact constructively with Sinkhorn-based distributional critics, improving sample efficiency and stability.

In this work we introduce MaxSink, which integrates a simple, interpretable family of reward transformations we call ``To the Max'' with Sinkhorn distributional critics. We provide: (i) a formalization of the transform and its potential-based variant that preserves optimality, (ii) a Sinkhorn-based distributional Bellman operator with implementation-level details, (iii) theoretical analyses that quantify bias and contraction properties, and (iv) a comprehensive empirical protocol and ablation study designed to evaluate interactions across environments and hyperparameters.
...
\textbf{Reward shaping and potential-based methods.} Potential-based shaping \cite{ng1999policy} guarantees policy invariance and remains a foundational result for shaping literature. Recent work has explored learned and task-specific shaping signals (heuristics or learned progress estimators) to accelerate sparse-goal learning (e.g., hindsight experience replay (HER) \cite{andrychowicz2017hindsight} combined with shaping in robotics contexts). Additional approaches include curriculum-based shaping for hierarchical tasks \cite{florensa2017reverse} and learned shaping via auxiliary networks \cite{harutyunyan2019hindsight}. Our To-the-Max transform builds on these by providing a simple, interpretable progress-based bonus that can be made potential-based for theoretical guarantees.

\textbf{Distributional RL.} Distributional approaches (C51 \cite{bellemare2017distributional}, QR-DQN \cite{dabney2018distributional}, implicit quantile networks, and particle-based methods) show improved stability and richer value estimates; these have been applied to both discrete and continuous control. Particle-based distributional critics capture multimodality which is useful when multiple strategies exist for achieving goals. Recent advances include Stein distributional methods \cite{rowland2019stein} and neural variants for high-dimensional spaces \cite{yang2019fully}.

\textbf{Optimal transport and Sinkhorn losses.} Entropic OT and Sinkhorn divergences \cite{cuturi2013sinkhorn,feydy2019interpolating} are increasingly used as robust, geometry-aware losses in ML. Recent works applied OT to fit distributions in generative modeling and in distributional RL style settings \cite{genevay2018learning}; our work builds on these ideas by combining OT with reward shaping. Sinkhorn-based distributional critics \cite{abdullah2020wasserstein} have shown promise in stabilizing training compared to raw Wasserstein losses.

\textbf{Shaping vs distributional losses.} To our knowledge, a systematic study of how explicit reward transformations (including potential-based variants) interact with OT-based distributional critics has not been performed; MaxSink fills this gap and provides diagnostic tools to diagnose beneficial vs harmful interactions. While some papers combine shaping with expectation-based critics \cite{devlin2012dynamic}, the synergy with distributional methods remains underexplored. We hypothesize that shaping reduces distributional variance, enabling more stable OT computations.
...
Computing a Sinkhorn loss for a single pair of particle sets of size $N$ requires $\mathcal{O}(N^2)$ pairwise cost computations per batch element; hence the per-update complexity is $\mathcal{O}(B N^2 + \text{model\_cost})$, where $B$ is the minibatch size. In practice, sample sizes $N\le64$ and batching strategies, combined with geomloss/KeOps acceleration, keep runtime feasible for the experiments we propose.
...
We present a set of formal statements that quantify the interaction between reward transformations and distributional Sinkhorn updates. Proof sketches and full derivations are deferred to Appendix A.

\subsection{Bias Bound (formal)}
\begin{lemma}[Bias bound]
Let $T$ be an arbitrary reward transform and $\Delta_r(s,a)=T(r)-r$ with $\|\Delta_r\|_\infty\le\delta$. Then for any policy $\pi$ and starting state $s$:
\begin{equation}
|V'^\pi(s)-V^\pi(s)| \le \frac{\delta}{1-\gamma}.
\end{equation}
\end{lemma}
\begin{proof}[Proof sketch]
The result follows by linearity of expectation and bounding the accumulated per-step transformation error by the geometric series $\sum_{t\ge0}\gamma^t\delta$; full details are in Appendix A.1.
\end{proof}

\subsection{Contraction and Lipschitz Properties}
\begin{lemma}[TD contraction under Lipschitz transforms]
If $T$ is $L_T$-Lipschitz in its scalar reward input, then the transformed TD operator contracts with factor at most $\gamma L_T$. If $L_T\le 1$, contraction is not worsened by shaping.
\end{lemma}
\begin{proof}[Sketch]
The transformed Bellman update is a composition of the original Bellman operator with $T$ applied to immediate rewards; Lipschitz composition yields contraction factor $\gamma L_T$. See Appendix A.2 for a formal derivation.
\end{proof}

\subsection{Sinkhorn Sensitivity and Variance}
We provide an informal sensitivity statement: if the reward transform concentrates mass (reducing target variance), the expected Sinkhorn cost between predicted and target particle clouds decreases in expectation, improving gradient signal-to-noise. Appendix A.3 sketches a bound relating the reduction in sample variance to an upper bound on expected Sinkhorn loss under bounded costs. Empirically, this motivates searching for values of $\beta$ that densify reward mass without saturating returns.
...
We evaluate MaxSink on a suite of sparse-reward environments spanning different challenges:
\begin{itemize}
  \item \textbf{Maze (MiniGrid)}: 2D navigation with sparse goal rewards. Progress measured as Euclidean distance reduction to goal.
  \item \textbf{Fetch (MuJoCo)}: Robotic manipulation tasks (reach, push, pick-and-place) with binary success rewards. Progress based on object-goal distance.
  \item \textbf{Procgen}: Procedurally generated levels with sparse rewards. Progress approximated via episode length or auxiliary signals.
  \item \textbf{MuJoCo Control}: Continuous control tasks with shaped rewards converted to sparse for testing.
\end{itemize}
Each environment provides a progress function $p(s,a,s')$ (e.g., distance reduction) used in the To-the-Max transform. We report both raw environment rewards and transformed rewards to validate shaping effects.
...
\begin{itemize}
  \item Potential-based vs non-potential shaping variants to measure bias effects
  \item Different progress metrics (e.g., binary progress vs continuous distance)
  \item Sinkhorn vs Wasserstein (non-entropic) losses for gradient quality comparison
  \item Varying regularization strength and its interaction with reward variance
\end{itemize}
...
\\subsection{Broader Implications and Societal Impact}
MaxSink contributes to the broader goal of making RL more sample-efficient and applicable to real-world sparse-reward tasks, such as robotics manipulation or autonomous navigation. By reducing the need for extensive human-designed reward engineering, our method could accelerate deployment in safety-critical domains. However, as with any reward modification technique, there is a risk of unintended behavior if shaping signals are misaligned with true objectives. We advocate for rigorous evaluation on raw rewards and encourage the use of our potential-based variant to minimize such risks. In terms of computational equity, while OT adds overhead, our fallback mechanisms ensure accessibility even without specialized hardware.

\\subsection{Future Work}
Several directions remain open: (i) extending MaxSink to multi-agent settings where shaping must account for cooperative or competitive dynamics, (ii) integrating learned progress estimators (e.g., via auxiliary networks) to make the transform adaptive, (iii) applying MaxSink to offline RL by transforming relabeled rewards in datasets, (iv) theoretical extensions to risk-sensitive variants using CVaR in both shaping and action selection, and (v) scaling to high-dimensional observations with learned representations for the progress metric.
...
\subsection{A.1 Proof of Lemma 1 (Bias bound)}
Let $\Delta_r(s,a)=T(r)-r$ and assume $|\Delta_r|\le\delta$. For any policy $\pi$ and starting state $s$:
\begin{align*}
|V'^\pi(s)-V^\pi(s)| &= \left|\mathbb{E}_\pi\sum_{t\ge0}\gamma^t \Delta_r(s_t,a_t)\right| \\
&\le \sum_{t\ge0}\gamma^t \delta = \frac{\delta}{1-\gamma}.
\end{align*}
This completes the proof of the bias bound used in Section 3.

\subsection{A.2 Lipschitz contraction derivation}
We show the TD operator with an $L_T$-Lipschitz transform $T$ contracts with factor at most $\gamma L_T$ by direct manipulation of the Bellman operator and applying Lipschitz composition bounds; full derivation is provided here (omitted for brevity in main text).

\subsection{A.3 Sinkhorn sensitivity sketch}
We sketch how a reduction in variance of target distributions in expectation implies a reduction in expected Sinkhorn loss under bounded pairwise costs. Key technical steps rely on bounding differences in cost matrices by squared differences of particle values and using Lipschitz continuity of the OT potentials with respect to perturbations in measures (see \cite{feydy2019interpolating} for supporting lemmas).

\section{Appendix B: Experimental Details and Reproducibility}
\paragraph{Configuration files} All experiment configs are available in `configs/` and include explicit seed lists, per-run random seeds, and system info logging.
\paragraph{Plot scripts} Scripts to generate each figure are provided in `scripts/` and take checkpoint directories as input. Each script saves PNGs compatible with the LaTeX placeholders in the main text.
\paragraph{Compute} Document GPU type, number, and approximate hours in the experiment metadata files; include `pip freeze` output alongside each run.
