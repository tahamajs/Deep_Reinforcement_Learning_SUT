...
The repository includes lightweight scripts and unit tests to validate core components. To reproduce basic checks locally (Python 3.10+):
\begin{enumerate}
  \item Create and activate a virtual environment and install dependencies: \texttt{python -m venv .venv && source .venv/bin/activate && python -m pip install -U pip torch numpy}
  \item Run unit tests: \texttt{python -m pytest tests/ -q} to verify network shapes, policy losses and a short MCTS smoke test.
  \item Run a small demo which exercises the policy and a short MCTS run: \texttt{python scripts/demo_run_ma_ezv2.py}.
\end{enumerate}

For full-scale experiments (SMAC, MPE), please adapt the configuration in \texttt{configs/ma_ezv2_default.yaml} and scale the search budget, seeds and runtime to match available compute. Always save checkpoints, configs and random seeds to ensure reproducibility.
\subsection{Metrics}
...
\begin{itemize}
  \item \textbf{Gumbel vs. standard PUCT}: Gumbel improves exploration, leading to 10\% higher win rates in early training.
  \item \textbf{Prefix loss}: Removing $L_z$ increases value prediction error by 2x, degrading performance.
  \item \textbf{Factored vs. joint}: Factored scales better for $N>3$ agents, with minimal performance drop.
  \item \textbf{Consistency loss}: Improves latent consistency, reducing training instability.
\end{itemize}
