...
python -m pip install -r dev-requirements.txt  # for test/dev tooling (optional)
```

- Run an environment check script that reports missing packages and install hints:

```bash
bash scripts/check_env.sh
```

- Run the minimal smoke run to validate forward passes (requires torch/gymnasium installed):

```bash
# use the project venv and ensure the CA14 package folder is on PYTHONPATH
...
- Run tests using the helper script (sets PYTHONPATH to CA14 and uses the venv python):

```bash
bash scripts/run_tests.sh
```

(Alternatively, set PYTHONPATH manually: `PYTHONPATH=paperAssignments/Assignments1-50/CA14 python3 -m pytest paperAssignments/Assignments1-50/CA14/tests -q`.)
