...
- `paperAssignments/Assignments1-50/CA12/pictures/fig_eval_returns.png` — Eval return curve (raw + smoothed). By default the script uses a rolling median smoothing window (10% of points); you can post-process or change the window inside `analysis/plot_results.py`.
- `paperAssignments/Assignments1-50/CA12/pictures/fig_losses.png` — Training loss curves (if you provide a loss CSV). The plotting script supports plotting multiple loss columns and a log y-axis.

Notes on confidence intervals and bootstrapping:
- If you produce multiple seeds' `eval_returns.csv` files, you can compute per-step bootstrap CIs and plot shaded regions. We recommend aggregating into a single CSV with columns `steps,seed,avg_return` and then using `seaborn.lineplot(..., ci='boot', n_boot=1000)` or compute the median and percentile bands yourself.
