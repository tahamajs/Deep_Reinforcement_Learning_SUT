...
def main():
    ...
    trig_cfg = TriggerConfig(
        cooldown=int(getattr(planner_cfg, "cooldown", 10)),
        trigger_td=float(getattr(planner_cfg, "trigger_td", 0.7)),
        trigger_unc=float(getattr(planner_cfg, "trigger_unc", 0.2)),
        trigger_ent_low=float(getattr(planner_cfg, "trigger_entropy_low", 0.3)),
        trigger_ent_high=float(getattr(planner_cfg, "trigger_entropy_high", 2.0)),
    )

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    buffer = CheckpointBuffer(
        capacity=int(getattr(planner_cfg, "buffer_size", 1024)), device=device
    )
    ...
    print(
        f"Starting training scaffold for {total_steps} steps. Planner enabled={getattr(planner_cfg, 'enabled', False)}"
    )
    ...
    while step < total_steps:
        ...
        # check trigger
        if getattr(planner_cfg, "enabled", False) and should_trigger(
