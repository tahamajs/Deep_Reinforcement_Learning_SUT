...
- [x] README ≥1000 lines (this doc).
- [x] Implement lambda-returns or backward traces with IS clipping. ✅ (see `src/losses.py` and unit tests)
- [x] Hidden recompute in training loop. ✅ (`critic_loss_lambda` supports recompute and uses current networks for per-batch recomputation)
- [x] Logging of returns and basic metrics. ✅ (`train_sac_full.py` logs `returns_mean`; `src/utils.compute_staleness` added)
- [ ] Ablations executed (lambda, clip, seq length, refresh). ⚠️ (requires running the experiments; not executed in this commit)
- [ ] Plots and tables generated. ⚠️ (requires experiment outputs)
...
2. Install minimal dev deps: `pip install torch pytest` (or use `requirements-dev.txt` if available).
3. From the `CA7` package run: `pytest -q` which runs the smoke/unit tests under `tests/`.

**How to build the report (LaTeX)**

- With a TeX system installed, run: `pdflatex report.tex` (twice) inside `CA7/` to produce `report.pdf`.
- `report_neurips.tex` is a conference-style variant prepared for NeurIPS formatting.

_This README now reflects implemented code and unit tests. To finish the evaluation checklist, run the full experiments and generate plots/tables (instructions and config templates are in the document)._ 

---

## Maintenance notes

Edits included in this commit:
- Added `src/config.py` (compact `Config` dataclass) for consistent defaults and test compatibility.
- Added `src/utils.compute_staleness` for hidden-state diagnostics.
- Enhanced `train_sac_full.py` to log `returns_mean` to CSV for easier plotting.
- Added `tests/test_utils.py` covering the `compute_staleness` utility.
- Updated `report.tex` and `report_neurips.tex` to document how to run tests and build the PDF.

If you'd like, I can add example experiment configs and a plotting helper to generate the paper figures from `train_log.csv` in a follow-up change.
