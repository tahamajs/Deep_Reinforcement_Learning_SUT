...
-**Math:**\( Q*{\text{risk}}(s,a)=\frac{1}{\beta}\log \mathbb{E}*{s'}[\exp(\beta (r+\gamma V(s')))] \); β<0 risk-averse.
...
-**Math:**\( Z\_\tau(s,a)\approx f(\psi(s)\odot \phi(\tau)) \) extended to k-dimensional rewards; Sinkhorn regularizer on joint outputs.
...
-**Math:**\( \mathcal{L}_{DPO}=-\log\sigma(\beta \log\frac{\pi_{ref}(y*w|x)}{\pi(y_w|x)} - \beta \log\frac{\pi*{ref}(y_l|x)}{\pi(y_l|x)}) \).
