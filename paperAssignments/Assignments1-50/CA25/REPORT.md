...
### Metrics and formulas to report
...
```tex
\mathrm{Precision}_c = \frac{\mathrm{TP}_c}{\mathrm{TP}_c + \mathrm{FP}_c}, \quad \mathrm{Recall}_c = \frac{\mathrm{TP}_c}{\mathrm{TP}_c + \mathrm{FN}_c}
```

### Metrics file (JSON schema)
It is useful to save a `metrics.json` file in the output folder with keys for the metrics per run. Example contents:
...
We provide a small utility script to compute metrics from predictions and targets and save this `metrics.json` and a confusion matrix image (for classification). See `scripts/compute_metrics.py`.
