...
def compute_gae(rewards: torch.Tensor, values: torch.Tensor, dones: torch.Tensor, gamma: float, lam: float = 0.95) -> torch.Tensor:
    ...
    for t in range(T - 1, -1, -1):
        ...
        nextvalues = values[:, t + 1] if t + 1 < values.shape[1] else torch.zeros(B, device=values.device)
        delta = rewards[:, t] + gamma * nextvalues * nextnonterminal - values[:, t]
        advantages[:, t] = lastgaelam = delta + gamma * lam * nextnonterminal * lastgaelam
    return advantages


def policy_value_losses(logp: torch.Tensor, advantages: torch.Tensor, values: torch.Tensor, returns: torch.Tensor, value_coef: float = 0.5) -> Dict[str, torch.Tensor]:
...
def kl_divergence_from_logits(logits_p: torch.Tensor, logits_q: torch.Tensor) -> torch.Tensor:
