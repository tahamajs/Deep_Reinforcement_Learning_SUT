...
def plot_reward_histograms(raw_rewards: Sequence[float],
                           transformed_rewards: Sequence[float],
                           save_path: str = "pictures/reward_hist.png") -> None:
...
def plot_sinkhorn_loss(losses: Sequence[float],
                       save_path: str = "pictures/sinkhorn_loss.png") -> None:
...
def plot_particle_pca(pred_particles: np.ndarray,
                      target_particles: np.ndarray,
                      save_path: str = "pictures/particles_pca.png",
                      sample: Optional[int] = 500) -> None:
    """
    ...
    plt.scatter(xp_coords[:, 0], xp_coords[:, 1], s=10, alpha=0.6, label="predicted", c="C0")
    plt.scatter(yp_coords[:, 0], yp_coords[:, 1], s=10, alpha=0.6, label="target", c="C1")
    plt.legend()
...
if __name__ == "__main__":
    ...
    transformed = np.clip(raw + rng.normal(scale=0.1, size=raw.shape) + (raw > 0) * 0.4, 0.0, 1.4)
