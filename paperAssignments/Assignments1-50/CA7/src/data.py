...
class SequenceReplayBuffer:
    ...
    def __init__(self, obs_dim: int, action_dim: int, seq_len: int, max_size: int = 1000):
    ...
    def add(self, obs_seq: np.ndarray, act_seq: np.ndarray, rew_seq: np.ndarray, done_seq: np.ndarray, beh_logp: Optional[np.ndarray] = None):
        """
        Add a single sequence to the buffer. Arrays expected shapes:
            ...
            beh_logp: [L] or None
        """
        self._buf.append((obs_seq.astype(np.float32), act_seq.astype(np.float32), rew_seq.astype(np.float32), done_seq.astype(np.float32), None if beh_logp is None else beh_logp.astype(np.float32)))
    ...
    def sample_batch(self, batch_size: int, device: str = "cpu"):
        ...
        """
        idx = np.random.choice(len(self._buf), size=batch_size, replace=len(self._buf) < batch_size)
        ...
        if logps[0] is None:
            ...
            return obs, acts, rews, dones, torch.tensor(np.stack(logps, axis=0), device=device)
