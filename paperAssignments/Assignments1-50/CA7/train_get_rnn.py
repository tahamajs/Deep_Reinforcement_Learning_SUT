...
def build_dummy_buffer(cfg: Config, n_sequences: int = 200) -> SequenceReplayBuffer:
    ...
    for _ in range(n_sequences):
        ...
        buf.add(obs, acts, rews, dones, beh_logp=None)
    return buf


def train_step(critic, target_critic, opt, batch, cfg: Config):
    obs, acts, rews, dones, beh_logp = batch
    loss, returns = critic_loss_lambda(critic, target_critic, obs, acts, rews, dones, beh_logp, cfg.gamma, cfg.lam, c_rho=cfg.c_rho)
    opt.zero_grad()
    loss.backward()
...
def main():
    ...
    buf = build_dummy_buffer(cfg)

    # simple loop
    for step in range(10):
        batch = buf.sample_batch(cfg.batch_size, device=str(device))
        loss = train_step(critic, target_critic, opt, batch, cfg)
        if step % 2 == 0:
            # soft update
