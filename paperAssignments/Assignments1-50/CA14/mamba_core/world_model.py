...
class RSSM(nn.Module):
    ...
    def __init__(self, obs_dim: int, act_dim: int, deter_dim: int = 200, stoch_dim: int = 32, morph_dim: int = 16) -> None:
        super().__init__()
        ...
        self.reward_head = nn.Sequential(nn.Linear(deter_dim + stoch_dim + morph_dim, 128), nn.ELU(), nn.Linear(128, 1))
        self.discount_head = nn.Sequential(nn.Linear(deter_dim + stoch_dim + morph_dim, 64), nn.ELU(), nn.Linear(64, 1))
    ...
    def observe_step(self, h: torch.Tensor, a: torch.Tensor, z_m: torch.Tensor, obs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """One step observe: update h using previous z,a and compute posterior over z.
        Returns new (h, z, mu_post, logvar_post)
        """
        inp = torch.cat([z_m, a, h.new_zeros(h.size(0), 0)], dim=-1) if False else torch.cat([h.new_zeros(h.size(0), 0)], dim=-1)
        ...
        h_new = self.gru(torch.cat([a, z_m, torch.zeros_like(h)[:, :0]], dim=-1) if False else torch.cat([a, z_m], dim=-1), h)
    ...
    def imagine_step(self, h: torch.Tensor, a: torch.Tensor, z_m: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def decode_obs(self, h: torch.Tensor, z: torch.Tensor, z_m: torch.Tensor) -> torch.Tensor:
        feat = torch.cat([h, z, z_m], dim=-1)
        return self.obs_decoder(feat)

    def predict_reward(self, h: torch.Tensor, z: torch.Tensor, z_m: torch.Tensor) -> torch.Tensor:
        feat = torch.cat([h, z, z_m], dim=-1)
        return self.reward_head(feat).squeeze(-1)

    def predict_discount(self, h: torch.Tensor, z: torch.Tensor, z_m: torch.Tensor) -> torch.Tensor:
        feat = torch.cat([h, z, z_m], dim=-1)
...
class WorldModel(nn.Module):
    ...
    def observe(self, h: torch.Tensor, a: torch.Tensor, z_m: torch.Tensor, obs: torch.Tensor):
