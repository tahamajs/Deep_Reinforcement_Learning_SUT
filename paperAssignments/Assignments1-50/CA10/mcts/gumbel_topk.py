...
def gumbel_noise(shape: Tuple[int, ...], device=None, dtype=torch.float32) -> torch.Tensor:
...
def topk_factored(per_agent_logits: List[torch.Tensor], k_each: int, max_beam: int = 64) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    joint_indices = topk_idx_per_agent[0].unsqueeze(1)  # (B, 1, k) treat k as candidates along last dim
    ...
    for i in range(1, N):
        ...
        idx_rep = idx_i.unsqueeze(1).expand(-1, beam, -1).reshape(batch, beam * idx_i.shape[1], 1)
        # combine scores
        score_rep = score_i.unsqueeze(1).expand(-1, beam, -1).reshape(batch, beam * idx_i.shape[1])
        joint_indices = torch.cat([beam_exp, idx_rep], dim=-1)  # (B, beam*k, i+1)
        joint_scores = (joint_scores.unsqueeze(2).expand(-1, -1, score_i.shape[1]).reshape(batch, -1) + score_rep)
        # prune to max_beam
