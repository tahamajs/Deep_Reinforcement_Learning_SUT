...
This repository contains a minimal, self-contained demonstration of a policy/value training loop using synthetic data. The aim is not to present novel research but to provide a reproducible scaffold students can extend into a short project (compare baselines, add a regularizer, or try a model-based component). The included `train` function is a small end-to-end example that trains a policy and value network using synthetic trajectories and saves checkpoints and metrics for later analysis.
...
- `report.md` summarizing assumptions, experiments and how to reproduce results
- `report.tex` (LaTeX) and a `Makefile` to build a PDF report (`make -C paperAssignments/Assignments1-50/CA21 report.pdf`)

## Usage ✅

Quick commands to run the demo and tests locally:

1. Run the quick training demo (CPU-friendly):

```
python -m src.train --epochs 2 --batch-size 8
```
...
3. Open the interactive demo notebook:
...
| input_dim | 8 | dimension of observation vectors |
| hidden_dim | 64 | MLP hidden dimension |
| action_dim | 4 | discrete action cardinality |
| lr | 1e-3 | learning rate for optimizers |
| batch_size | 32 | training batch size |
| epochs | 10 | number of training epochs |

> **Note:** the `configs/debug.yaml` file contains smaller values suitable for CI and quick runs (e.g., `batch_size: 8`, `epochs: 2`).

### Requirements

This project requires Python 3.10+ and the following packages (install in a virtualenv):

```
python -m pip install --upgrade pip
python -m pip install torch numpy pytest matplotlib
```

If you plan to open the notebooks interactively, install Jupyter as well:
...
**Note:** This repository uses synthetic data for educational demos and is not intended as a production-ready RL benchmark.
