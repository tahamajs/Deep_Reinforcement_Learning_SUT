...
class SyntheticDataset(Dataset):
    ...
    def __init__(self, num_samples: int = 256, input_dim: int = 8, action_dim: int = 4, seed: Optional[int] = None):
        super().__init__()
        rng = torch.Generator()
        if seed is not None:
            rng.manual_seed(seed)
        self.observations = torch.randn(num_samples, input_dim, generator=rng)
        self.actions = torch.randint(low=0, high=action_dim, size=(num_samples,), generator=rng)
        self.rewards = torch.randn(num_samples, )
