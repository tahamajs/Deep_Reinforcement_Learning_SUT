...
- Placeholder figures are provided in `results/figures/`:
  - `a2c_learning_curve.svg` — illustrative A2C learning curve (replace with your run output). This figure displays the episodic reward averaged per evaluation batch (smoothed). X axis: environment steps. Y axis: episode reward (higher is better). 
  - `bandit_rewards.svg` — illustrative bandit reward curve showing cumulative or per-step reward under an eps-greedy policy; useful to verify the agent learns to pick the best arm.
  - `a2c_loss_decomposition.svg` — placeholder showing policy and value loss trends over training; use this to inspect optimization stability and relative magnitudes.
  - `a2c_entropy.svg` — placeholder showing policy entropy over training; used to monitor exploration decay or collapse.
...
This will produce PNGs suitable for inclusion in `report.tex`. After generating PNGs, run `make report` to rebuild `report.pdf` including the new figures.
