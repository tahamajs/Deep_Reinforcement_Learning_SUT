...
A full LaTeX paper is included as `paper.tex` with bibliography `references.bib` and a build script at `scripts/build_paper.sh`. Build locally with `make paper` or `./scripts/build_paper.sh` (requires pdflatex/latexmk and typical TeX packages).
