...
class ActorCriticEnsemble(nn.Module):
    ...
    def __init__(self, obs_dim: int, action_dim: int, hidden_dim: int = 64, ensemble_size: int = 3):
        super().__init__()
    ...
    def forward(self, obs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        values = torch.stack([head(trunk).squeeze(-1) for head in self.value_heads], dim=0)
        return logits, values
