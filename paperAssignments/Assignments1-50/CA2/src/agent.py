...
def build_optimizer(model: torch.nn.Module, optim_name: str = "sophia", lr: float = 3e-4, **kwargs) -> torch.optim.Optimizer:
    """
    ...
    if optim_name.lower() == "sophia":
        ...
        for name, p in model.named_parameters():
            ...
            if name.endswith(".bias") or "bn" in name.lower() or "batchnorm" in name.lower():
                param_groups.append({"params": [p], "hessian_estimator": "squared_grad", "beta1": 0.9})
            else:
...
def example_training_step(model: torch.nn.Module, optimizer: torch.optim.Optimizer, batch: Dict[str, torch.Tensor], use_amp: bool = True):
    """
    ...
    def closure():
        ...
        if scaler is not None:
            scaler.scale(loss).backward(create_graph=getattr(optimizer, \"hessian_estimator\", None) == \"hutchinson\")
        else:
            loss.backward(create_graph=getattr(optimizer, \"hessian_estimator\", None) == \"hutchinson\")
        return loss

    # If the optimizer requires closure (Hutchinson), provide it to step()
    if hasattr(optimizer, "param_groups") and any(g.get("hessian_estimator", "squared_grad") == "hutchinson" for g in optimizer.param_groups):
        loss = closure()
