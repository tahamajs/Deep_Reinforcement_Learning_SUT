...
def plot_learning_curves(logs: list, label_col='label', x='env_step', y='return', smooth=10, out_file=None):
    ...
    for df in logs:
        ...
        if 'seed' in df.columns:
            for seed, g in df.groupby('seed'):
                g = g.sort_values(x)
                plt.plot(g[x], g[y].rolling(smooth, min_periods=1).mean(), alpha=0.25, label=f'{group}-s{seed}')
        else:
            df_sorted = df.sort_values(x)
            plt.plot(df_sorted[x], df_sorted[y].rolling(smooth, min_periods=1).mean(), alpha=0.8, label=str(group))
...
def plot_norms(log_df: pd.DataFrame, out_file=None, smooth=50):
    ...
    if 'grad_norm' in log_df.columns:
        ...
        plt.plot(g['step'], g['grad_norm'].rolling(smooth, min_periods=1).mean(), label='grad norm')
    if 'hessian_mean' in log_df.columns:
        h = log_df.sort_values('step')
        plt.plot(h['step'], h['hessian_mean'].rolling(smooth, min_periods=1).mean(), label='hessian mean')
    plt.yscale('log')
