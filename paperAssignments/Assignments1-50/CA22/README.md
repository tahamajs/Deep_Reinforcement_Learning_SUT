...
A sample report template and a submission checklist are provided in `reports/REPORT_TEMPLATE.md` (also available as a LaTeX template: `reports/REPORT_TEMPLATE.tex`). An extended example report in LaTeX is provided at `reports/EXAMPLE_REPORT.tex` and can be compiled with the helper script `reports/build_report.sh`.

Your final submission should include:

- A short report (2–4 pages) following the template (PDF or LaTeX source).
- Config file(s) used for experiments (YAML).
- Key scripts or a notebook that reproduces the main results.
- Figures and captions saved to `outputs/figures/` (referenced from the report).

Building the LaTeX report (example):

```sh
# from repo root
./reports/build_report.sh reports/EXAMPLE_REPORT.tex
# or use latexmk directly: latexmk -pdf reports/EXAMPLE_REPORT.tex
```

If you do not have LaTeX installed, the Markdown report (`reports/REPORT_TEMPLATE.md`) is sufficient; otherwise, include both PDF and source `.tex` for full reproducibility.
