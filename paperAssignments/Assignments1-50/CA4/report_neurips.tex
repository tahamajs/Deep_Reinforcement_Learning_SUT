...
\author{Anonymous authors for NeurIPS submission}
\date{}
...
We situate Dist-SCAS relative to Conservative Q-Learning (CQL) \citep{cql2020}, distributional critics (QR-DQN, TQC), lower-expectile approaches (LEQ), and SCAS-style state correction. Dist-SCAS synthesizes these strands: distributional critics provide explicit tail-risk signals, while SCAS regularizers prevent the policy from drifting into OOD states even when actions appear promising under value estimates.
...
% Placeholder figures (will be replaced with real plots)
\begin{figure}[ht]
\centering
\begin{tikzpicture}
  \draw[fill=gray!10] (0,0) rectangle (6,3);
  \node at (3,1.5) {Quantile fan plot (placeholder)};
\end{tikzpicture}
\caption{Quantile fan across training (placeholder).}
\label{fig:quantile_fan}
\end{figure}
...
\begin{tikzpicture}
  \draw[fill=gray!10] (0,0) rectangle (6,3);
  \node at (3,1.5) {AntMaze overlay (placeholder)};
\end{tikzpicture}
\caption{AntMaze dataset vs policy rollouts (placeholder).}
\label{fig:antmaze_overlay}
\end{figure}
...
The quantile Huber loss used in training is derived from the quantile regression asymmetric weighting combined with Huber robustification; see the code and README for full details.
