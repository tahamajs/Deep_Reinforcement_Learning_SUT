...
class DQNVisualizer:
    ...
    def plot_learning_curves(
        ...
        for i, (agent_name, result) in enumerate(results.items()):
            ...
            ax1.plot(
                smoothed, label=agent_name, color=colors[i % len(colors)], linewidth=2
            )
        ...
        ax4 = axes[1, 1]
        stability_scores = [
            np.std(result["scores"][-100:]) for result in results.values()
        ]
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.save_dir, save_name), dpi=300, bbox_inches="tight"
        )
        plt.show()

    def plot_q_value_analysis(
        self, agent, env, num_states: int = 100, save_name: str = "q_value_analysis.png"
    ):
        ...
        for action in range(q_values_array.shape[1]):
            ax1.hist(
                q_values_array[:, action], alpha=0.6, label=f"Action {action}", bins=20
            )
    ...
    def plot_hyperparameter_sensitivity(
        ...
        param_name: str,
        save_name: str = "hyperparameter_sensitivity.png",
    ):
        ...
        try:
            ...
            ax2.plot(
                numeric_values, scores, "o-", linewidth=2, markersize=8, color="red"
            )
    ...
    def plot_robustness_analysis(
        ...
        if "seed_robustness" in robustness_results:
            ...
            ax1.axhline(
                ...
                linestyle="--",
                alpha=0.7,
                label=f"+1σ: {mean_score + std_score:.1f}",
            )
            ax1.axhline(
                mean_score - std_score,
                color="orange",
        ...
        if "scale_robustness" in robustness_results:
            ...
            ax2.semilogx(
                scales, scale_scores, "o-", linewidth=2, markersize=8, color="red"
            )
    ...
    def create_summary_report(
        self, results: Dict[str, Any], save_name: str = "summary_report.png"
    ):
        ...
        if variant_names:
            ...
            for i, (name, data) in enumerate(variants.items()):
                ...
                axes[1, 0].plot(
                    smoothed, label=name, color=colors[i % len(colors)], linewidth=2
                )
        ...
        if variant_names:
            ...
            bars = axes[2, 2].bar(
                metric_names, metric_values, alpha=0.7, color="purple"
            )
...
class DQNAnalyzer:
    ...
    def analyze_convergence(
        self, scores: List[float], target: float = 180, window: int = 20
    ) -> Dict[str, Any]:
        ...
        convergence_episode = (
            converged_idx[0] if len(converged_idx) > 0 else len(smoothed)
        )
    ...
    def analyze_exploration_efficiency(
        self, epsilon_history: List[float], scores: List[float]
    ) -> Dict[str, Any]:
        """Analyze exploration efficiency"""
        # Find exploration phases
        high_exploration = np.array(epsilon_history) > 0.5
        medium_exploration = (np.array(epsilon_history) > 0.1) & (
            np.array(epsilon_history) <= 0.5
        )
        ...
        return {
            "high_exploration_performance": (
                np.mean(high_exp_scores) if len(high_exp_scores) > 0 else 0
            ),
            "medium_exploration_performance": (
                np.mean(medium_exp_scores) if len(medium_exp_scores) > 0 else 0
            ),
            "low_exploration_performance": (
                np.mean(low_exp_scores) if len(low_exp_scores) > 0 else 0
            ),
            "exploration_efficiency": (
                np.mean(low_exp_scores) / np.mean(high_exp_scores)
                if len(high_exp_scores) > 0 and len(low_exp_scores) > 0
                else 0
            ),
        }
...
def run_benchmark(
    agent_class, env_name: str = "CartPole-v1", episodes: int = 200, num_runs: int = 5
) -> Dict[str, Any]:
