...
class PrioritizedExperienceReplay:
    ...
    def sample(self, batch_size: int) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, torch.Tensor]:
        ...
        # Sample indices
        indices = np.random.choice(len(self.experiences), size=batch_size, p=probabilities)
        ...
        batch = {
            ...
            "next_states": torch.stack([self.experiences[i]["next_state"] for i in indices]),
...
class TaskBalancedReplay:
    ...
    def __init__(
        ...
        # Per-task buffers
        self.task_buffers = [deque(maxlen=capacity // num_tasks) for _ in range(num_tasks)]
