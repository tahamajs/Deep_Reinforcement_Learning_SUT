...
class RSSMCore(nn.Module):
    ...
    def __init__(
        ...
        self.rnn = nn.GRUCell(
            input_size=self.state_dim + self.action_dim + self.embed_dim,
    ...
    def observe(
        self, embed: torch.Tensor, action: torch.Tensor, state: Dict[str, torch.Tensor]
    ...
    def imagine(
        self, action: torch.Tensor, state: Dict[str, torch.Tensor]
    ) -> Dict[str, torch.Tensor]:
        """
        ...
        hidden = self.rnn(
            torch.cat([state["stoch"], action, zero_embed], dim=1), state["hidden"]
        )
...
class WorldModel(nn.Module):
    ...
    def _kl_divergence(
        ...
        kl = (
            var1 / var2 + (mean2 - mean1).pow(2) / var2 + torch.log(std2 / std1) - 1
...
class MPCPlanner:
    ...
    def plan(self, initial_state: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        mean = torch.zeros(
            ...
            self.horizon,
            self.action_dim,
            device=initial_state["hidden"].device,
        )
        std = torch.ones(
            batch_size,
            self.horizon,
    ...
    def _evaluate_sequences(
        ...
        gamma = 0.99
        returns = torch.zeros(
            batch_size * n_candidates, device=initial_state["hidden"].device
        )

        for t in range(self.horizon):
            discount = gamma**t
            continue_discount = (
                torch.prod(continues[:, : t + 1], dim=1) if t > 0 else continues[:, 0]
            )
...
class ImaginationAugmentedAgent(nn.Module):
    ...
    def __init__(
        ...
        self.imagination_encoder = nn.Sequential(
            nn.Linear(
                world_model.state_dim + world_model.hidden_dim + 1, 128
            ),  # state + reward
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
    ...
    def forward(
        self, obs: torch.Tensor, use_imagination: bool = True
    ) -> Dict[str, torch.Tensor]:
        """
        ...
        if not use_imagination:
            return {"action": mf_action, "value": value, "imagination_features": None}
        ...
        with torch.no_grad():
            ...
            current_state = self.world_model.rssm.observe(
                embed, dummy_action, initial_state
            )
    ...
    def _generate_imagination_features(
        ...
        imagination_actions = torch.randn(
            batch_size, horizon, self.action_dim, device=state["hidden"].device
        )
        ...
        for t in range(horizon):
            ...
            state_concat = torch.cat(
                [state_t["stoch"], state_t["hidden"], reward_t], dim=1
            )
