...
Given a fixed dataset \(\mathcal{D} = \{(s_i, a_i, r_i, s_i')\}_{i=1}^N\) collected by one or more behavior policies \(\pi_\beta\), the goal is to learn a policy \(\pi\) that maximizes expected return:
...
The dataset \(\mathcal{D}\) is generated by behavior policy \(\pi_\beta\):

\[
(s_t, a_t) \sim d^{\pi_\beta}(s, a) = (1-\gamma)\sum_{t=0}^\infty \gamma^t P(s_t=s) \pi_\beta(a|s_t=s)
\]

where \(d^{\pi_\beta}\) is the discounted state-action visitation distribution.
...
**Problem:** If \((s', \arg\max_{a'} Q(s',a'))\) is not in \(\mathcal{D}\), the Q-value is **extrapolated** and potentially inaccurate.
...
\[
\epsilon_{t+1}(s,a) \leq \gamma \mathbb{E}_{s'}[\max_{a'} \epsilon_t(s',a')]
\]
...
\[
\min_Q \alpha \cdot \mathbb{E}_{s \sim \mathcal{D}}\left[\log \sum_a \exp(Q(s,a)) - \mathbb{E}_{a \sim \mathcal{D}}[Q(s,a)]\right] + \mathbb{E}_{(s,a,s') \sim \mathcal{D}}[(Q(s,a) - \mathcal{B}^\pi Q(s,a))^2]
\]
...
\[
J(\pi_{CQL}) \geq J(\pi_\beta) - \frac{2\gamma \epsilon}{(1-\gamma)^2}
\]
...
\[
Q(s,a) = r + \gamma \max_{a'} Q(s', a') \quad \text{← max causes OOD queries}
\]
...
where \(L_\tau(u) = |\tau - \mathbb{1}(u < 0)| u^2\).
...
\[
\pi(s) \in \arg\max_{a: \pi_\beta(a|s) > \delta} Q(s,a)
\]
...
1. Train a generative model \(G_\omega(s)\) to model \(\pi_\beta(a|s)\)
2. Sample actions from \(G_\omega\)
3. Apply small perturbations \(\xi\) to sampled actions
4. Select action with highest Q-value among perturbed samples

\[
\pi(s) = \arg\max_{a_i + \xi_i} Q(s, a_i + \xi_i), \quad a_i \sim G_\omega(s)
\]
...
\[
\max_\pi \mathbb{E}_{(s,a) \sim \mathcal{D}}[\exp(\beta A^\pi(s,a)) \cdot \log \pi(a|s)]
\]
...
\[
\max_\pi J(\pi) - \alpha \mathbb{E}_{s \sim \mathcal{D}}[D_{KL}(\pi(\cdot|s) || \pi_\beta(\cdot|s))]
\]
...
Train an ensemble of dynamics models \(\{T_\theta^i\}_{i=1}^K\):

\[
T_\theta^i(s_{t+1}|s_t, a_t)
\]
...
\[
\text{Score} = \frac{R_{\text{policy}} - R_{\text{random}}}{R_{\text{expert}} - R_{\text{random}}} \times 100
\]
...
\[
\pi^* = \arg\max_\pi \mathbb{E}_{d^\pi}[Q^\pi(s,a)]
\]
...
Let \(d_{TV}(d^\pi, d^{\pi_\beta})\) be the total variation distance between policy distributions.

**Online RL:** \(d_{TV}(d^{\pi_t}, d^{\pi_{t-1}}) \approx 0\) (small policy updates)

**Offline RL:** \(d_{TV}(d^\pi, d^{\pi_\beta})\) can be arbitrarily large

The value estimation error scales as:
\[
|\mathbb{E}_{d^\pi}[Q(s,a)] - \mathbb{E}_{d^\pi}[Q^*(s,a)]| = O\left(\frac{d_{TV}(d^\pi, d^{\pi_\beta})}{1-\gamma}\right)
\]
...
CQL's objective:
\[
\min_Q \underbrace{\mathbb{E}_{s \sim \mathcal{D}}\left[\log \sum_a \exp(Q(s,a))\right]}_{\text{Push down all Q-values}} - \underbrace{\mathbb{E}_{(s,a) \sim \mathcal{D}}[Q(s,a)]}_{\text{Boost dataset Q-values}}
\]
...
\[
\mathbb{E}_{\pi}[Q^\pi(s,a)] - \mathbb{E}_{\pi}[Q^{CQL}(s,a)] \leq C \cdot d_{TV}(d^\pi, d^{\pi_\beta})
\]
...
\[
\alpha^* = \arg\min_\alpha \mathbb{E}_{\pi}[Q_\alpha^{CQL}(s,a)] \text{ s.t. } \mathbb{E}_{\mathcal{D}}[Q_\alpha^{CQL}(s,a)] \geq r_{\text{target}}
\]
...
2. **Requires modeling \(\pi_\beta\):** May be difficult with mixed datasets
...
BR methods optimize:
\[
\max_\pi J(\pi) \text{ s.t. } D_{KL}(\pi || \pi_\beta) \leq \epsilon
\]

**Performance bound:**
\[
J(\pi_{BR}) \leq J(\pi_\beta) + C_1 \sqrt{\epsilon}
\]
...
**Advantages:**
1. **Policy improvement:** Can significantly outperform \(\pi_\beta\)
2. **No behavior modeling:** Doesn't need to estimate \(\pi_\beta\)
...
VR methods optimize:
\[
\max_\pi \mathbb{E}_{d^\pi}[Q^{\text{conservative}}(s,a)]
\]

**Performance bound:**
\[
J(\pi_{VR}) = J(\pi^*) - C_2 \cdot \text{conservatism\_gap}
\]
...
|--------|------------------------|----------------------|
| **Policy Improvement** | Limited (near \(\pi_\beta\)) | Significant (toward \(\pi^*\)) |
| **Stability** | Very stable | Moderately stable |
| **Hyperparameters** | Few, easy to tune | More, sensitive |
| **Dataset Requirements** | Single, consistent \(\pi_\beta\) | Mixed datasets OK |
...
\[
\max_\pi \mathbb{E}[Q^{\text{conservative}}(s,a)] - \alpha D_{KL}(\pi || \pi_\beta)
\]
...
- \(N_{\text{effective}} = N_{\text{real}} + N_{\text{synthetic}}\)
...
- Hybrid model: \(T(s,a) = T_{\text{physics}}(s,a) + T_{\text{learned}}(s,a)\)
...
- Explicit planning: \(\arg\max_{a_1,...,a_H} \sum_{t=1}^H r_t\)
...
\[
N_{\text{MF}} = O\left(\frac{1}{\epsilon^2(1-\gamma)^4}\right)
\]

**Sample complexity of model-based:**
\[
N_{\text{MB}} = O\left(\frac{1}{\epsilon(1-\gamma)^3}\right)
\]
...
\[
\epsilon_{\text{total}} = \epsilon_{\text{model}} + \epsilon_{\text{planning}}
\]
...
\[
J(\pi) \approx \frac{\sum_{i=1}^N w_i R_i}{\sum_{i=1}^N w_i}
\]

where \(w_i = \prod_{t=0}^T \frac{\pi(a_t^i|s_t^i)}{\pi_\beta(a_t^i|s_t^i)}\)
...
\[
J(\pi) \approx \frac{1}{N}\sum_{i=1}^N \left[w_i(R_i - Q(s_0^i, a_0^i)) + V(s_0^i)\right]
\]
...
3. **Sanity Check:** Ensure \(J(\pi) \geq J(\pi_\beta)\) (should improve)
...
3. **Safety Metrics:**
   - Probability of improvement: \(P(J(\pi) > J(\pi_\beta))\)
