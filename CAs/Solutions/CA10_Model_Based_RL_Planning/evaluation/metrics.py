...
class PerformanceMetrics:
    ...
    def calculate_learning_metrics(
        ...
        sample_efficiency = self._episodes_to_target(
            episode_rewards, target_performance
        )
        ...
        convergence_episodes = self._episodes_to_target(
            episode_rewards, 0.95 * final_performance
        )

        # Improvement ratio
        improvement_ratio = (final_performance - initial_performance) / (
            abs(initial_performance) + 1e-8
        )
    ...
    def calculate_model_metrics(
        self, model_accuracy: float, model_uncertainty: float = 0.0
    ) -> Dict[str, float]:
    ...
    def calculate_planning_metrics(
        self, planning_times: List[float], planning_costs: List[float]
    ) -> Dict[str, float]:
    ...
    def calculate_computational_metrics(
        self, training_times: List[float], memory_usage: List[float] = None
    ) -> Dict[str, float]:
    ...
    def aggregate_metrics(
        self, multiple_runs: List[Dict[str, Any]]
    ) -> Dict[str, float]:
    ...
    def create_metrics_report(self, metrics_dict: Dict[str, Any]) -> str:
        ...
        if any(
            key.startswith(
                ("final_performance", "learning_efficiency", "sample_efficiency")
            )
            for key in metrics_dict.keys()
        ):
            report += "🎯 Learning Performance:\n"
            report += (
                f"  Final Performance: {metrics_dict.get('final_performance', 0):.3f}\n"
            )
        ...
        if any(
            key.startswith(("training_time", "memory")) for key in metrics_dict.keys()
        ):
        ...
        if any(key.startswith("model") for key in metrics_dict.keys()):
            ...
            report += (
                f"  Model Confidence: {metrics_dict.get('model_confidence', 0):.3f}\n\n"
            )

        return report

    def visualize_metrics(
        self, metrics_history: List[Dict[str, float]], save_path: str = None
    ) -> plt.Figure:
        ...
        if "stability" in df.columns:
            axes[1, 1].plot(
                df.index, df["stability"], "m-", linewidth=2, label="Stability"
            )
