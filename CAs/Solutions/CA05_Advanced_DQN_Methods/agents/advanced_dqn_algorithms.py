...
class NoisyDQNAgent:
    ...
    def __init__(self, state_dim: int, action_dim: int, lr: float = 1e-3,
                 gamma: float = 0.99, buffer_size: int = 100000, batch_size: int = 32,
                 target_update_freq: int = 1000, device: str = "cpu"):
...
class DistributionalDQNAgent:
    ...
    def __init__(self, state_dim: int, action_dim: int, lr: float = 1e-3,
                 gamma: float = 0.99, buffer_size: int = 100000, batch_size: int = 32,
                 target_update_freq: int = 1000, num_atoms: int = 51,
                 v_min: float = -10.0, v_max: float = 10.0, device: str = "cpu"):
        ...
        # Networks
        self.q_network = CategoricalDQN(state_dim, action_dim, num_atoms=num_atoms,
                                       v_min=v_min, v_max=v_max).to(device)
        self.target_network = CategoricalDQN(state_dim, action_dim, num_atoms=num_atoms,
                                           v_min=v_min, v_max=v_max).to(device)
        self.optimizer = torch.optim.Adam(self.q_network.parameters(), lr=lr)
    ...
    def update(self) -> float:
        ...
        with torch.no_grad():
            ...
            target_atoms = rewards.unsqueeze(1) + self.gamma * self.q_network.z.unsqueeze(0) * (~dones).unsqueeze(1)
...
class MultiStepDQNAgent:
    ...
    def __init__(self, state_dim: int, action_dim: int, lr: float = 1e-3,
                 gamma: float = 0.99, buffer_size: int = 100000, batch_size: int = 32,
                 target_update_freq: int = 1000, n_step: int = 3, device: str = "cpu"):
...
class HierarchicalDQNAgent:
    ...
    def __init__(self, state_dim: int, action_dim: int, goal_dim: int = 4,
                 lr: float = 1e-3, gamma: float = 0.99, buffer_size: int = 100000,
                 batch_size: int = 32, target_update_freq: int = 1000, device: str = "cpu"):
        ...
        self.controller_optimizer = torch.optim.Adam(self.controller.parameters(), lr=lr)
