...
class AdvancedVisualizer:
    ...
    def plot_q_value_heatmap_3d(self, agent, env, save_path: Optional[str] = None):
        ...
        fig = go.Figure(data=[
            go.Scatter3d(
                ...
                text=[f"State: ({s[0]}, {s[1]})<br>Q-Value: {q:.3f}" for s, q in zip(states, q_values[:, 0])],
    ...
    def plot_learning_curves_comparison(self, results: Dict[str, Any], save_path: Optional[str] = None):
        """Compare learning curves of different agents"""
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=("Episode Rewards", "Episode Lengths", "Loss Curves", "Success Rate"),
        ...
        for i, (agent_name, data) in enumerate(results.items()):
            ...
            if 'episode_rewards' in data:
                ...
                if window_size > 0:
                    success_rate = pd.Series(data['episode_rewards']).rolling(window_size).apply(
                        lambda x: (x > 0).mean()
                    ).fillna(0)
        ...
        fig.update_layout(
            title="Comprehensive Learning Analysis",
            height=800,
            showlegend=True
        )
    ...
    def plot_hyperparameter_sensitivity(self, results: Dict[str, Any], save_path: Optional[str] = None):
        ...
        for config_name, result in results.items():
            if 'config' in result and 'evaluation_results' in result:
                ...
                data.append({
                    ...
                    'buffer_size': config.get('agent', {}).get('buffer_size', 10000),
        ...
        fig = make_subplots(
            rows=2, cols=3,
            subplot_titles=("Learning Rate", "Gamma", "Epsilon Start", 
                           "Buffer Size", "Batch Size", "Performance Summary"),
    ...
    def plot_network_architecture_analysis(self, agent, save_path: Optional[str] = None):
        ...
        for name, module in agent.q_network.named_modules():
            if isinstance(module, (nn.Linear, nn.Conv2d)):
                layer_info = {
                    ...
                    'input_size': module.in_features if hasattr(module, 'in_features') else 'N/A',
                    'output_size': module.out_features if hasattr(module, 'out_features') else 'N/A',
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=("Network Architecture", "Weight Distributions", 
                           "Bias Distributions", "Parameter Count"),
    ...
    def plot_experience_replay_analysis(self, replay_buffer, save_path: Optional[str] = None):
        ...
        fig = make_subplots(
            rows=2, cols=3,
            subplot_titles=("Reward Distribution", "Action Distribution", "Done Distribution",
                           "State Space Coverage", "Reward vs Action", "Experience Timeline"),
        ...
        if states.shape[1] >= 2:
            fig.add_trace(
                go.Scatter(
                    ...
                    mode='markers',
                    name='State Coverage',
                    marker=dict(size=3, opacity=0.5, color=rewards, colorscale='Viridis')
                ),
                row=2, col=1
            )
...
class HyperparameterOptimizer:
    ...
    def grid_search(self, param_grid: Dict[str, List], num_episodes: int = 500) -> Dict[str, Any]:
    ...
    def random_search(self, param_distributions: Dict[str, Any], n_iter: int = 50, 
                     num_episodes: int = 500) -> Dict[str, Any]:
        ...
        for i in range(n_iter):
            ...
            for param_name, distribution in param_distributions.items():
                if isinstance(distribution, list):
                    ...
                    params[param_name] = random.uniform(distribution[0], distribution[1])
                else:
    ...
    def bayesian_optimization(self, param_bounds: Dict[str, Tuple], n_iter: int = 30,
                             num_episodes: int = 500) -> Dict[str, Any]:
    ...
    def _evaluate_params(self, params: Dict[str, Any], num_episodes: int) -> float:
        ...
        return np.mean(episode_rewards[-100:]) if len(episode_rewards) >= 100 else np.mean(episode_rewards)
...
class PerformanceAnalyzer:
    ...
    def analyze_convergence(self, episode_rewards: List[float], window_size: int = 100) -> Dict[str, Any]:
        ...
        convergence_analysis = {
            ...
            'final_performance': np.mean(rewards[-window_size:]) if len(rewards) >= window_size else np.mean(rewards),
        ...
        if len(rolling_avg) > window_size * 2:
            ...
            if len(recent_avg) > 0:
                ...
                if std_dev < np.mean(recent_avg) * 0.1:  # Less than 10% variation
                    convergence_analysis['convergence_episode'] = len(rewards) - window_size
                    convergence_analysis['stability'] = 1 - (std_dev / np.mean(recent_avg))
        
        # Calculate improvement rate
        if len(rewards) > window_size:
            early_performance = np.mean(rewards[:window_size])
            late_performance = np.mean(rewards[-window_size:])
            convergence_analysis['improvement_rate'] = (late_performance - early_performance) / abs(early_performance)
        
        return convergence_analysis
    
    def analyze_sample_efficiency(self, episode_rewards: List[float], target_performance: float = 0.8) -> Dict[str, Any]:
    ...
    def analyze_exploration_efficiency(self, actions: List[int], rewards: List[float]) -> Dict[str, Any]:
        ...
        reward_action_corr = np.corrcoef(actions, rewards)[0, 1] if len(actions) > 1 else 0
