        ...
        if mode == "human":
            ...
            for i, goal in enumerate(self.goals):
                circle = patches.Circle(
                    (goal[1], goal[0]), 0.3, color=colors[i % len(colors)]
                )
                ax.add_patch(circle)
            ...
            for i, pos in enumerate(self.agent_positions):
                triangle = patches.RegularPolygon(
                    (pos[1], pos[0]), 3, radius=0.2, color=colors[i % len(colors)]
                )
    ...
    def _generate_obstacles(self):
        ...
        for _ in range(num_obstacles):
            while True:
                ...
                if (
                    obs != self.agent_pos
                    and obs != self.goal_pos
                    and obs not in self.obstacles
                ):
                    self.obstacles.append(obs)
                    break
...
class HierarchicalEnvironment(gym.Env):
    ...
    def _calculate_reward(self) -> Tuple[float, bool, Dict[str, Any]]:
        ...
        if not done:
            ...
            distance = abs(self.agent_pos[0] - target[0]) + abs(
                self.agent_pos[1] - target[1]
            )
        ...
        info = {
            "agent_pos": self.agent_pos,
            "current_subgoal": (
                self.subgoals[self.current_subgoal_idx]
                if self.current_subgoal_idx < len(self.subgoals)
                else None
            ),
