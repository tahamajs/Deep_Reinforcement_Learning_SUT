...
class FederatedServer:
    ...
    def aggregate_parameters(
        self, client_parameters: Dict[str, Dict[str, torch.Tensor]]
    ) -> Dict[str, torch.Tensor]:
        ...
        total_weight = sum(
            self.client_weights[client_id] for client_id in client_parameters.keys()
        )
    ...
    def communication_round(
        self, participating_clients: List[str] = None, local_epochs: int = 5
    ) -> Dict[str, Any]:
...
class FederatedRLAggregator:
    ...
    def add_client(self, client_id: str, lr: float = 1e-3) -> FederatedClient:
        ...
        client = FederatedClient(
            client_id, self.state_dim, self.action_dim, lr, self.device
        )
    ...
    def train_federation(self, num_rounds: int = None) -> Dict[str, Any]:
        ...
        for round_idx in range(num_rounds):
            ...
            # Communication round
            round_results = self.server.communication_round(
                participating_clients, self.local_epochs
            )
