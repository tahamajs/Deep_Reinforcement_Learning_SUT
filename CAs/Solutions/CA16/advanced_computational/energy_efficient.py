...
class EarlyExitNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor, force_full: bool = False) -> Tuple[torch.Tensor, int, float]:
    ...
    def get_energy_statistics(self) -> Dict[str, Any]:
        ...
        early_exit_rate = total_early_exits / max(1, self.exit_statistics['total_forward_passes'])
...
class AdaptiveComputation(nn.Module):
    ...
    def get_computation_statistics(self) -> Dict[str, Any]:
        ...
        layer_usage_rates = [usage / max(1, self.computation_statistics['total_forward_passes']) 
...
class EnergyEfficientRL(nn.Module):
    ...
    def __init__(self, state_dim: int, action_dim: int, method: str = 'early_exit',
