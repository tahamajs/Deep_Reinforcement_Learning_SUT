...
class ContinualLearningEnvironment(Env):
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        if self.task_step_count > 0 and self.task_step_count % self.task_switch_frequency == 0:
            self._switch_task()
...
class TaskSwitchingEnvironment(Env):
    ...
    def get_task_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": self.task_successes[self.current_task] / max(1, self.task_attempts[self.current_task]),
        }
