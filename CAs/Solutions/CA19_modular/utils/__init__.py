...
class PerformanceTracker:
    ...
    def update_episode(
        self, episode_reward: float, episode_length: int, metrics: Dict[str, Any]
    ):
    ...
    def get_summary_stats(self) -> Dict[str, float]:
        ...
        if self.episode_rewards:
            ...
            if len(self.episode_rewards) >= 20:
                ...
                stats["learning_progress"] = (late_performance - early_performance) / (
    ...
    def plot_training_progress(self, save_path: Optional[str] = None):
        ...
        if len(self.episode_rewards) >= 20:
            axes[0, 0].plot(
                np.convolve(self.episode_rewards, np.ones(20) / 20, mode="same"),
                color="darkblue",
                linewidth=2,
...
class ExperimentManager:
    ...
    def run_experiment(
        self, experiment_idx: int, num_episodes: int = 50, verbose: bool = False
    ) -> Dict[str, Any]:
    ...
    def generate_comparison_report(self, save_path: Optional[str] = None) -> str:
        ...
        for exp_name, result in self.results.items():
            ...
            if "avg_quantum_fidelity" in stats:
                report.append(
                    f"  Quantum Fidelity: {stats['avg_quantum_fidelity']:.4f}"
                )
...
def benchmark_quantum_vs_classical(
    quantum_agent, classical_agent, environment, num_episodes: int = 50
) -> Dict[str, Any]:
    """
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            action, action_info = quantum_agent.select_action(
                state, epsilon=0.1, quantum_enabled=True
            )
            next_state, reward, done, info = environment.step(action)
            episode_reward += reward
            state = next_state
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            action, action_info = classical_agent.select_action(
                state, epsilon=0.1, quantum_enabled=False
            )
            next_state, reward, done, info = environment.step(action)
            episode_reward += reward
            state = next_state
