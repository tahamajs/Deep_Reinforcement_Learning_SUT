...
class ExperienceReplay:
    ...
    def __init__(self, capacity: int = 10000, device: str = "cpu"):
        ...
        # Statistics
        self.stats = {"total_experiences": 0, "task_counts": {}, "avg_reward": 0.0}
    ...
    def add(self, experience: Experience):
        ...
        self.stats["avg_reward"] = (total_reward + experience.reward) / self.stats[
            "total_experiences"
        ]
...
class PrioritizedExperienceReplay:
    ...
    def __init__(
        ...
        # Statistics
        self.stats = {"total_experiences": 0, "task_counts": {}, "avg_priority": 0.0}
    ...
    def add(self, experience: Experience):
        ...
        total_priority = self.stats["avg_priority"] * (
            self.stats["total_experiences"] - 1
        )
        self.stats["avg_priority"] = (total_priority + priority) / self.stats[
            "total_experiences"
        ]
...
class ContinualExperienceReplay:
    ...
    def __init__(
        self, capacity: int = 10000, task_capacity: int = 1000, device: str = "cpu"
    ):
        self.capacity = capacity
    ...
    def sample(
        self, batch_size: int, task_id: int = None, use_task_weights: bool = True
    ) -> List[Experience]:
        ...
        if task_id is not None and task_id in self.task_buffers:
            ...
            if len(task_experiences) >= batch_size:
                ...
                global_sample = random.sample(
                    list(self.global_buffer), min(remaining, len(self.global_buffer))
                )
        ...
        if use_task_weights and self.task_weights:
            ...
            return random.sample(
                list(self.global_buffer), min(batch_size, len(self.global_buffer))
            )
    ...
    def _weighted_sample(self, batch_size: int) -> List[Experience]:
        ...
        for _ in range(batch_size):
            ...
            task_id = np.random.choice(
                list(task_probs.keys()), p=list(task_probs.values())
            )
    ...
    def compute_forgetting_measure(
        self, task_id: int, current_performance: float
    ) -> float:
