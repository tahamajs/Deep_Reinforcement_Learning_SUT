...
class ComprehensiveDQNAnalyzer:
    ...
    def compare_dqn_variants(self):
        ...
        variants = {
            ...
            'Dueling DQN (Max)': DuelingDQNAgent(
                state_dim=state_dim, action_dim=action_dim,
                dueling_type='max', lr=1e-3, epsilon_decay=0.995, buffer_size=15000
            )
        }
    ...
    def _plot_variant_comparison(self, results):
        ...
        ax.bar(x + width/2, eval_means, width, yerr=eval_stds,
               label='Evaluation Performance', alpha=0.7, color=colors)
    ...
    def generate_comprehensive_report(self):
        ...
        if 'basic_dqn' in self.results:
            ...
            print(f"\nBasic DQN Results:")
            print(f"  Final Training Performance: {basic['eval_performance']['mean_reward']:.2f} ± {basic['eval_performance']['std_reward']:.2f}")
            print(f"  Training Time: {basic['training_time']:.1f} seconds")
            print(f"  Success Rate: {(basic['eval_performance']['mean_reward'] >= 195):.1%}")
        ...
        if 'variant_comparison' in self.results:
            ...
            for name, data in variants.items():
                ...
                print(f"  {name}: {perf['mean_reward']:.1f} ± {perf['std_reward']:.1f} "
                      f"({time_taken:.1f}s)")
