...
class DQNAgent:
    ...
    def __init__(self, state_dim: int, action_dim: int, lr: float = 1e-3,
                 ...
                 replay_buffer_size: int = 10000, batch_size: int = 64,
                 target_update_freq: int = 10, hidden_dim: int = 128):
    ...
    def evaluate(self, env: gym.Env, num_episodes: int = 10, max_steps: int = 1000) -> Dict[str, float]:
...
class DuelingDQNAgent(DQNAgent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.q_network = DuelingQNetwork(self.state_dim, self.action_dim, kwargs.get('hidden_dim', 128))
        self.target_network = DuelingQNetwork(self.state_dim, self.action_dim, kwargs.get('hidden_dim', 128))
        self.target_network.load_state_dict(self.q_network.state_dict())
        self.target_network.eval()

        # Reinitialize optimizer with new parameters
        self.optimizer = optim.Adam(self.q_network.parameters(), lr=kwargs.get('lr', 1e-3))
...
def train_dqn_agent(agent_class: type, env_name: str = 'CartPole-v1', episodes: int = 500,
                   **agent_kwargs) -> Dict[str, List[float]]:
    """Train a DQN agent"""
    ...
    for episode in range(episodes):
        ...
        if (episode + 1) % 50 == 0:
            ...
            print(f"Episode {episode+1:3d} | Average Score: {avg_score:6.1f} | Average Loss: {avg_loss:.4f} | Epsilon: {agent.epsilon:.3f}")

    env.close()
    return {'scores': scores, 'losses': avg_losses, 'epsilon_history': agent.epsilon_history}

def compare_dqn_variants(env_name: str = 'CartPole-v1', episodes: int = 300) -> Dict[str, Dict]:
...
def plot_dqn_comparison(results: Dict[str, Dict], save_path: Optional[str] = None):
    ...
    axes[1,1].bar(methods, score_variances, alpha=0.7, edgecolor='black', color='orange')
...
def hyperparameter_optimization_study(env_name: str = 'CartPole-v1', episodes: int = 200):
    ...
    for i, arch in enumerate(architectures):
        ...
        result = train_dqn_agent(DoubleDQNAgent, env_name, episodes,
                               hidden_dim=arch['hidden_dim'], lr=arch['lr'])
    ...
    for i, sched in enumerate(exploration_schedules):
        print(f"  Schedule {i+1}: Start={sched['eps_start']}, End={sched['eps_end']}, Decay={sched['eps_decay']}")
        result = train_dqn_agent(DoubleDQNAgent, env_name, episodes,
    ...
    return {
        ...
        'exploration_schedules': exploration_results
    }
...
def robustness_analysis(env_name: str = 'CartPole-v1', episodes: int = 300):
    ...
    axes[1].plot(reward_scales, scale_scores, 'o-', linewidth=2, markersize=8, color='red')
    ...
    return {
        'seed_robustness': robustness_results,
        'scale_robustness': scale_results
    }
