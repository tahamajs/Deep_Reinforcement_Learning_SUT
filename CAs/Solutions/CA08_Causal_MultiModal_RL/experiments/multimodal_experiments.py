...
class MultiModalExperiments:
    ...
    def test_fusion_strategies(
        ...
        for strategy in fusion_strategies:
            ...
            for episode in range(n_episodes):
                ...
                for step in range(20):  # Max steps per episode
                    ...
                    if strategy == "early":
                        ...
                        text_feat = processed_obs[
                            wrapper.visual_dim : wrapper.visual_dim + wrapper.text_dim
                        ]
                        state_feat = processed_obs[
                            wrapper.visual_dim + wrapper.text_dim :
                        ]
                        fused_features = np.concatenate(
                            [visual_feat, text_feat, state_feat]
                        )
                ...
                processed_obs = wrapper.process_observation(obs)
                modality_contributions["visual"].append(
                    np.linalg.norm(processed_obs[: wrapper.visual_dim])
                )
                modality_contributions["textual"].append(
                    np.linalg.norm(
                        processed_obs[
                            wrapper.visual_dim : wrapper.visual_dim + wrapper.text_dim
                        ]
                ...
                modality_contributions["state"].append(
                    np.linalg.norm(
                        processed_obs[wrapper.visual_dim + wrapper.text_dim :]
                    )
                )
            ...
            print(
                f"{strategy} fusion: Avg reward = {avg_reward:.3f} ± {std_reward:.3f}"
            )
    ...
    def test_modality_importance(
        self, env_size: int = 6, render_size: int = 64, n_episodes: int = 100
    ) -> Dict[str, Dict[str, Any]]:
        ...
        for modality in modalities:
            ...
            print(
                f"{modality}: Avg reward = {avg_reward:.3f} ± {std_reward:.3f}, "
    ...
    def plot_results(self, results: Dict[str, Any], save_path: Optional[str] = None):
        ...
        if "fusion_strategies" in results:
            ...
            avg_rewards = [
                fusion_results[strategy]["avg_reward"] for strategy in strategies
            ]
            std_rewards = [
                fusion_results[strategy]["std_reward"] for strategy in strategies
            ]

            x = np.arange(len(strategies))
            bars = axes[0, 0].bar(
                x, avg_rewards, yerr=std_rewards, capsize=5, alpha=0.8
            )
        ...
        if "missing_modality" in results:
            ...
            avg_rewards = [
                missing_results[scenario]["avg_reward"] for scenario in scenarios
            ]
        ...
        if "modality_importance" in results:
            ...
            feature_norms = [
                importance_results[mod]["avg_feature_norm"] for mod in modalities
            ]
            ...
            axes[1, 0].bar(
                x - width / 2, avg_rewards, width, label="Avg Reward", alpha=0.8
            )
            axes[1, 0].bar(
                x + width / 2, feature_norms, width, label="Feature Norm", alpha=0.8
            )
        ...
        if "fusion_strategies" in results:
            ...
            for strategy in strategies:
                ...
                if (
                    len(contributions["visual"]) > 1
                    and len(contributions["textual"]) > 1
                ):
                    corr = np.corrcoef(
                        contributions["visual"], contributions["textual"]
                    )[0, 1]
