...
def run_world_model_experiment(config):
    ...
    plot_world_model_analysis(
        world_model, train_data, device, f"World Model Analysis - {config['env_name']}"
    )
...
def test_rollout(world_model, env, device, rollout_steps):
    ...
    for step in range(rollout_steps):
        ...
        with torch.no_grad():
            next_obs_pred, reward_pred = world_model.predict_next_state_and_reward(
    ...
    print(
        f"Rollout completed: {len(rollout_states)-1} steps, total reward: {sum(rollout_rewards):.2f}"
    )
