...
from environments.sequence_environment import SequenceEnvironment, MultiSequenceEnvironment
...
def run_rssm_experiment(config: Dict[str, Any]) -> tuple[RSSM, RSSMTrainer]:
    ...
    for epoch in range(config['train_epochs']):
        # Train epoch
        train_losses = trainer.train_epoch(train_data, num_batches=config.get('batches_per_epoch', 50))
        
        # Validate
        val_losses = trainer.evaluate(val_data, num_batches=config.get('val_batches', 20))
    ...
    # Final evaluation
    test_losses = trainer.evaluate(test_data, num_batches=config.get('test_batches', 20))
...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/rssm_config.json',
                       help='Path to configuration file')
    parser.add_argument('--env', type=str, default='sequence_environment',
