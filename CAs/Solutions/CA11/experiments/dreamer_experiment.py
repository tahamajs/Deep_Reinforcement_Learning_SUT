...
def run_dreamer_experiment(config):
    ...
    for episode in tqdm(range(config["train_episodes"]), desc="Training Episodes"):
        ...
        episode_tensors = {
            ...
            "next_observations": torch.FloatTensor(
                np.array(episode_data["next_observations"])
            ).to(device),
        }
    ...
    plot_performance_comparison(
        eval_rewards, random_rewards, f"Performance Comparison - {config['env_name']}"
    )
...
def create_and_pretrain_world_model(env, train_data, config, device):
    ...
    dynamics = LatentDynamicsModel(
        ...
        stochastic=config["stochastic_dynamics"],
    ).to(device)
    reward_model = RewardModel(latent_dim, action_dim, config["reward_hidden_dims"]).to(
        device
    )
    ...
    for step in tqdm(range(config["wm_pretrain_steps"]), desc="Pre-training"):
        ...
        batch = {
            ...
            "next_observations": train_data["next_observations"][batch_indices].to(
                device
            ),
        }
...
if __name__ == "__main__":
    ...
    print(
        f"Random Policy - Mean: {np.mean(random_rewards):.2f}, Std: {np.std(random_rewards):.2f}"
    )
    print(
        f"Improvement: {((np.mean(eval_rewards) - np.mean(random_rewards)) / abs(np.mean(random_rewards)) * 100):.1f}%"
    )
