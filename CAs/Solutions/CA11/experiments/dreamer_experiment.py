...
def run_dreamer_experiment(config: Dict[str, Any]) -> tuple[DreamerAgent, Dict[str, Any]]:
    """
    ...
    print(f"Collected {len(world_model_data['observations'])} transitions for world model")
    ...
    for epoch in range(config['world_model_epochs']):
        ...
        indices = np.random.choice(len(world_model_data['observations']), batch_size, replace=False)
        
        batch = {
            'observations': torch.FloatTensor(world_model_data['observations'][indices]).to(device),
            'actions': torch.FloatTensor(world_model_data['actions'][indices]).to(device),
            'next_observations': torch.FloatTensor(world_model_data['next_observations'][indices]).to(device),
            'rewards': torch.FloatTensor(world_model_data['rewards'][indices]).to(device)
        }
        ...
        if (epoch + 1) % config.get('world_model_print_interval', 50) == 0:
            print(f"World Model Epoch {epoch+1}: Loss = {losses['total_loss'].item():.4f}")
    ...
    print(f"Final evaluation: Avg Reward = {final_avg_reward:.2f}, Avg Length = {final_avg_length:.1f}")
    ...
    if config.get('save_plots', True):
        ...
        plt.tight_layout()
        plt.savefig(save_dir / "dreamer_episode_metrics.png", dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/dreamer_config.json',
    ...
    parser.add_argument('--dreamer_episodes', type=int, default=200,
                       help='Number of Dreamer training episodes')
    parser.add_argument('--world_model_epochs', type=int, default=100,
