...
class StateNormalizationWrapper(gym.Wrapper):
    ...
    def _update_stats(self, state):
        ...
        self.state_std = np.sqrt((1 - alpha) * self.state_std**2 + alpha * (state - self.state_mean)**2)
...
def create_cartpole_env(reward_scale: float = 1.0, normalize_states: bool = False) -> gym.Env:
...
def create_mountain_car_env(reward_scale: float = 1.0, normalize_states: bool = False) -> gym.Env:
...
def create_acrobot_env(reward_scale: float = 1.0, normalize_states: bool = False) -> gym.Env:
