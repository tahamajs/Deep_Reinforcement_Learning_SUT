...
def train_dqn_agent(env_name: str = 'CartPole-v1',
                   agent_type: str = 'dqn',
...
def plot_q_value_landscape(agent: DQNAgent, env_name: str = 'CartPole-v1', save_path: Optional[str] = None):
...
def plot_experience_replay_analysis(replay_buffer: Union[ReplayBuffer, PrioritizedReplayBuffer],
...
if __name__ == "__main__":
    ...
    best_variant = max(training_results.keys(),
    ...
    # Episode rewards
    axes[0,0].plot(episodes, training_results[best_variant]['episode_rewards'], alpha=0.7)
    axes[0,0].plot(episodes, pd.Series(training_results[best_variant]['episode_rewards']).rolling(50).mean(),
    ...
    # Episode lengths
    axes[0,1].plot(episodes, training_results[best_variant]['episode_lengths'], alpha=0.7)
    axes[0,1].plot(episodes, pd.Series(training_results[best_variant]['episode_lengths']).rolling(50).mean(),
                   linewidth=2, color='red', label='Rolling Mean (50)')
    ...
    # Exploration rate
    axes[1,1].plot(episodes, training_results[best_variant]['epsilons'], linewidth=2, color='green')
