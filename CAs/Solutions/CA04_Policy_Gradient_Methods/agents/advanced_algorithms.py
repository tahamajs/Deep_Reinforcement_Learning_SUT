...
class TRPOAgent:
    ...
    def __init__(self, state_size: int, action_size: int, lr: float = 0.001,
                 gamma: float = 0.99, kl_target: float = 0.01, 
                 max_kl: float = 0.02, cg_iters: int = 10, 
                 damping: float = 0.1):
    ...
    def compute_gae_advantages(self, rewards: List[float], values: List[torch.Tensor],
                              next_value: torch.Tensor = None, 
                              lambda_gae: float = 0.95) -> List[torch.Tensor]:
    ...
    def fisher_vector_product(self, states: torch.Tensor, vector: torch.Tensor) -> torch.Tensor:
        ...
        kl_grad = torch.autograd.grad(kl_div, self.policy_net.parameters(), 
                                     create_graph=True, retain_graph=True)
        
        # Compute Fisher-vector product
        kl_grad_flat = torch.cat([g.view(-1) for g in kl_grad])
        fisher_vector_product = torch.autograd.grad(kl_grad_flat, self.policy_net.parameters(),
    ...
    def update_policy(self, states: List[np.ndarray], actions: List[int],
                     advantages: List[torch.Tensor], old_log_probs: List[torch.Tensor]) -> Dict[str, float]:
        ...
        policy_grad = torch.autograd.grad(policy_loss, self.policy_net.parameters(),
                                        create_graph=True, retain_graph=True)
        ...
        for _ in range(10):
            ...
            kl_div = torch.sum(probs * torch.log(probs / (new_probs + 1e-8) + 1e-8), dim=-1).mean()
    ...
    def train(self, env, num_episodes: int = 1000, print_every: int = 100) -> Dict[str, List]:
        ...
        for episode in range(num_episodes):
            ...
            if (episode + 1) % print_every == 0:
                ...
                avg_policy_loss = np.mean(self.policy_losses[-print_every:]) if self.policy_losses else 0
                avg_value_loss = np.mean(self.value_losses[-print_every:]) if self.value_losses else 0
                avg_kl_div = np.mean(self.kl_divergences[-print_every:]) if self.kl_divergences else 0
...
class SACAgent:
    ...
    def __init__(self, state_size: int, action_size: int, lr: float = 0.0003,
                 ...
                 hidden_size: int = 256, target_entropy: float = None):
    ...
    def get_action(self, state: np.ndarray, evaluate: bool = False) -> Tuple[np.ndarray, torch.Tensor]:
    ...
    def train_episode(self, env) -> Tuple[float, int]:
        ...
        # Update networks
        self.update_critics(states_tensor, actions_tensor, rewards_tensor, 
    ...
    def train(self, env, num_episodes: int = 1000, print_every: int = 100) -> Dict[str, List]:
        ...
        for episode in range(num_episodes):
            ...
            if (episode + 1) % print_every == 0:
                avg_score = np.mean(scores[-print_every:])
                avg_actor_loss = np.mean(self.actor_losses[-print_every:]) if self.actor_losses else 0
                avg_critic_loss = np.mean(self.critic_losses[-print_every:]) if self.critic_losses else 0
                avg_alpha_loss = np.mean(self.alpha_losses[-print_every:]) if self.alpha_losses else 0
...
class DDPGAgent:
    ...
    def __init__(self, state_size: int, action_size: int, lr_actor: float = 0.001,
                 lr_critic: float = 0.002, gamma: float = 0.99, tau: float = 0.005,
                 noise_std: float = 0.1, hidden_size: int = 256):
    ...
    def train_episode(self, env) -> Tuple[float, int]:
        ...
        # Update networks
        self.update_critic(states_tensor, actions_tensor, rewards_tensor, 
                          next_states_tensor, dones_tensor)
        self.update_actor(states_tensor)
        self.soft_update()
