...
class TrainingPlotter:
    ...
    def plot_convergence_analysis(
        ...
        if target_performance:
            axes[0, 1].axhline(
                y=target_performance,
                color="red",
                linestyle="--",
                alpha=0.7,
                label=f"Target: {target_performance:.1f}",
            )
        ...
        axes[1, 0].boxplot(
            final_performances.values(), labels=final_performances.keys()
        )
...
class PerformanceVisualizer:
    ...
    def plot_performance_heatmap(
        self, results: Dict[str, Dict], save_name: Optional[str] = None
    ) -> None:
    ...
    def plot_statistical_comparison(
        self, results: Dict[str, List[float]], save_name: Optional[str] = None
    ) -> None:
        ...
        axes[0, 0].violinplot(
            [final_data[algo] for algo in final_data.keys()],
...
class PolicyVisualizer:
    ...
    def plot_action_distribution(
        ...
        plt.hist(
            all_actions, bins=range(max(all_actions) + 2), alpha=0.7, edgecolor="black"
        )
        plt.title(title, fontsize=16, fontweight="bold")
