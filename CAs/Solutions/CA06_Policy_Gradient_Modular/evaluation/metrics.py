...
class PerformanceAnalyzer:
    ...
    def has_converged(self, threshold: float = 0.01, min_episodes: int = 200) -> bool:
        ...
        return (recent_std / (recent_mean + 1e-8)) < threshold and abs(improvement) < threshold
...
class ConvergenceAnalyzer:
    ...
    def add_algorithm_results(self, algorithm_name: str, rewards: List[float], 
                            losses: List[float] = None, lengths: List[float] = None):
...
def calculate_sample_efficiency(rewards: List[float], target_performance: float, 
                              window_size: int = 50) -> Optional[int]:
...
def calculate_final_performance(rewards: List[float], window_size: int = 100) -> Dict[str, float]:
