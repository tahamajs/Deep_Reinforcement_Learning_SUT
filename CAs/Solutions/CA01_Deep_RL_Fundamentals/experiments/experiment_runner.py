...
class ExperimentRunner:
    ...
    def run_comprehensive_comparison(
        ...
        agent_configs = {
            ...
            "REINFORCE": {"state_size": 4, "action_size": 2, "lr": 1e-3},
            "Actor-Critic": {
                "state_size": 4,
                "action_size": 2,
                "lr_actor": 1e-3,
    ...
    def _run_algorithm_experiments(
        self,
        algorithm_name: str,
        config: Dict[str, Any],
        env,
        ...
        for run in range(n_runs):
            ...
            if algorithm_name == "DQN":
                ...
                start_time = time.time()
                scores = self._train_reinforce_agent(
                    agent, env_run, n_training_episodes
                )
            elif algorithm_name == "Actor-Critic":
                agent = ActorCriticAgent(**config)
                start_time = time.time()
                scores = self._train_actor_critic_agent(
                    agent, env_run, n_training_episodes
                )
    ...
    def _plot_training_curves(self):
        ...
        for i, (algorithm, results) in enumerate(self.experiment_results.items()):
            ...
            averaged_scores = np.mean(
                [scores[:min_length] for scores in all_scores], axis=0
            )
        ...
        plt.axhline(
            y=195, color="black", linestyle=":", alpha=0.7, label="CartPole Threshold"
        )
    ...
    def _plot_final_performance(self):
        ...
        bars = axes[0, 0].bar(
            algorithms, mean_scores, yerr=std_scores, capsize=5, alpha=0.7, color=colors
        )
    ...
    def _plot_sample_efficiency(self):
        ...
        for i, (algorithm, results) in enumerate(self.experiment_results.items()):
            ...
            if episodes_to_threshold:
                episodes_to_threshold = [
                    ep for ep in episodes_to_threshold if ep is not None
                ]
                if episodes_to_threshold:
                    plt.hist(
                        episodes_to_threshold,
    ...
    def _perform_statistical_tests(self):
        ...
        for i in range(n_algorithms):
            for j in range(i + 1, n_algorithms):
                ...
                print(
                    f"{alg1} vs {alg2}: t={t_stat:.3f}, p={p_value:.4f} {significance}"
                )
    ...
    def _save_detailed_results(self):
        ...
        for algorithm, results in self.experiment_results.items():
            ...
            for key, value in results.items():
                if (
                    ...
                    serializable_results[algorithm][key] = [
                        arr.tolist() for arr in value
                    ]
        ...
        for algorithm, results in self.experiment_results.items():
            ...
            summary_stats[algorithm] = {
                "mean_score": float(
                    np.mean([run["mean_score"] for run in eval_results])
                ),
                "std_score": float(np.std([run["mean_score"] for run in eval_results])),
                "mean_success_rate": float(
                    np.mean([run["success_rate"] for run in eval_results])
                ),
...
def run_quick_demo():
    ...
    results = runner.run_comprehensive_comparison(
        n_training_episodes=100, n_eval_episodes=20, n_runs=2
    )
