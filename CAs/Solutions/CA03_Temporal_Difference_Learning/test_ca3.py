...
from agents.exploration import ExplorationStrategies, BoltzmannQLearning, ExplorationExperiment
...
def test_q_learning_agent():
    ...
    assert greedy_action in env.get_valid_actions(env.start_state), "Greedy action should be valid"
...
def test_sarsa_agent():
    ...
    next_state = (0, 1)
    next_action = 'right'
    td_error = agent.update_q_sarsa(state, action, reward, next_state, next_action, False)
    assert state in agent.Q, "State should be added to Q-table"
...
def test_experiments():
    ...
    q_agent, V_optimal, q_policy, q_evaluation = experiment_q_learning(env, num_episodes=100)
    ...
    sarsa_agent, V_sarsa, sarsa_policy, sarsa_evaluation = experiment_sarsa(env, num_episodes=100)
    ...
    return td_agent, q_agent, sarsa_agent, V_td, V_optimal, V_sarsa, q_evaluation, sarsa_evaluation
...
def test_evaluation():
    ...
    comparison_results = compare_agents(agents_dict, env, num_episodes=30, save_dir="test_visualizations")
...
def run_comprehensive_test():
    ...
    try:
        ...
        td_agent2, q_agent2, sarsa_agent2, V_td2, V_optimal2, V_sarsa2, q_eval2, sarsa_eval2 = test_experiments()
        test_evaluation()
        test_visualization()
        test_model_utils()
