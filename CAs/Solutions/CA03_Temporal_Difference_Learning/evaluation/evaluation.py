...
def evaluate_agent(
    agent, env, num_episodes=100, save_results=True, save_dir="visualizations"
):
    """
...
def create_comparison_plots(results, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_dir, "agent_comparison.png"), dpi=300, bbox_inches="tight"
    )
    plt.show()
...
def create_performance_plots(agent, analysis, save_dir):
    ...
    if len(agent.episode_rewards) >= window:
        ...
        ax1.plot(
            moving_avg, color="red", linewidth=2, label=f"Moving Average ({window})"
        )
        ax1.legend()
    ...
    ax2 = axes[0, 1]
    ax2.hist(
        agent.episode_rewards, bins=30, alpha=0.7, color="green", edgecolor="black"
    )
    ax2.axvline(
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_dir, f"{agent.__class__.__name__}_performance_analysis.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
