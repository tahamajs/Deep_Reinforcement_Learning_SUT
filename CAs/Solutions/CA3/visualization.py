...
def compare_algorithms(
    ...
    for name, info in algorithms.items():
        if info["evaluation"]:
            ...
            print(
                f"{name:<12} {info['type']:<20} {avg_reward:<12.2f} {success_rate:<12.1f}%"
            )
        else:
    ...
    print(
        f"{'State':<10} {'TD(0)':<10} {'Q-Learning':<12} {'SARSA':<10} {'Q-S Diff':<10}"
    )
    print("-" * 55)
    ...
    for state in key_states:
        ...
        print(
            f"{str(state):<10} {td_val:<10.2f} {q_val:<12.2f} {s_val:<10.2f} {diff:<10.3f}"
        )
