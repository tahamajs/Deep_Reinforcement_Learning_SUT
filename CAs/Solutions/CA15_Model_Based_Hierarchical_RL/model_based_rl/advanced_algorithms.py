    ...
    def adapt_to_task(self, support_data, adaptation_steps=5):
        ...
        for step in range(adaptation_steps):
            ...
            for state, action, next_state, reward in support_data:
                pred_next_state, pred_reward = self.forward(state, action, adapted_params)
                loss = F.mse_loss(pred_next_state, next_state) + F.mse_loss(pred_reward, reward)
                total_loss += loss
...
class SafeModelBasedRL(nn.Module):
    ...
    def get_safe_action(self, state, deterministic=False):
        ...
        safety_violations = torch.sum(torch.relu(constraints - self.safety_threshold), dim=-1)
