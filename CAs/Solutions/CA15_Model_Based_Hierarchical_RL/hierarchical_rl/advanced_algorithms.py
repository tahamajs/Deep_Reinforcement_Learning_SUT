    ...
    def __init__(self, state_dim, action_dim, num_levels=3, hidden_dim=256):
        ...
        for level in range(num_levels):
            optimizer = optim.Adam(
                list(self.policies[level].parameters())
                + list(self.critics[level].parameters()),
                lr=3e-4,
            )
    ...
    def update_difficulty(self, success_rate):
        ...
        if success_rate > 0.8:
            self.difficulty_level = min(
                self.difficulty_level + self.curriculum_rate, self.max_difficulty
            )
        elif success_rate < 0.3:
            self.difficulty_level = max(
                self.difficulty_level - self.curriculum_rate, 0.0
            )
...
class ContinuousOptionCritic(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, num_options=4, hidden_dim=256):
        ...
        self.master_optimizer = optim.Adam(
            list(self.option_selection.parameters())
            + list(self.master_value.parameters()),
            lr=3e-4,
        )
