...
def plot_multi_modal_attention_patterns(agent=None, save_path=None):
    ...
    for i in range(len(modalities)):
        for j in range(len(modalities)):
            text = axes[0, 0].text(j, i, f'{avg_attention[i, j]:.2f}',
                                 ha="center", va="center", color="white", fontweight='bold')
    
    # Self-attention evolution over time
    for i, modality in enumerate(modalities):
        attention_to_self = attention_weights[:, i, i]
        axes[0, 1].plot(attention_to_self, label=f'{modality}→{modality}', linewidth=2, marker='o')
    ...
    axes[1, 0].hist(cross_attention, bins=20, alpha=0.7, edgecolor='black', color='skyblue')
    ...
    axes[1, 0].axvline(mean_cross, color='red', linestyle='--', linewidth=2, label=f'Mean: {mean_cross:.3f}')
    axes[1, 0].axvline(mean_cross + std_cross, color='orange', linestyle='--', alpha=0.7, label=f'+1σ: {mean_cross + std_cross:.3f}')
    axes[1, 0].axvline(mean_cross - std_cross, color='orange', linestyle='--', alpha=0.7, label=f'-1σ: {mean_cross - std_cross:.3f}')
    axes[1, 0].legend()
    ...
    p = np.poly1d(z)
    axes[1, 1].plot(range(time_steps), p(range(time_steps)), "r--", alpha=0.8, linewidth=2, label=f'Trend: {z[0]:.3f}x + {z[1]:.3f}')
    axes[1, 1].legend()
...
def multi_modal_fusion_strategy_comparison():
    ...
    strategies = ['Early Fusion', 'Late Fusion', 'Cross-Modal Attention', 'Hierarchical Fusion']
    ...
    for i, metric in enumerate(metrics):
        ...
        bars = axes[1, 0].bar(x + offset, performance_data[:, i], width, label=metric, alpha=0.8)
    ...
    scatter = axes[1, 1].scatter(complexity, overall_performance, s=200, alpha=0.7, c=range(len(strategies)), cmap='viridis')
    
    for i, strategy in enumerate(strategies):
        axes[1, 1].annotate(strategy, (complexity[i], overall_performance[i]), 
    ...
    p = np.poly1d(z)
    axes[1, 1].plot(complexity, p(complexity), "r--", alpha=0.8, linewidth=2, label=f'Trend: {z[0]:.3f}x + {z[1]:.3f}')
    axes[1, 1].legend()
