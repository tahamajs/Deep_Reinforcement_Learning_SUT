...
class CausalDiscoveryMetrics:
    ...
    def update(self, true_graph: nx.DiGraph, predicted_graph: nx.DiGraph, 
    ...
    def get_metrics(self) -> Dict[str, float]:
        ...
        return {
            ...
            'recall': recall,
            'f1_score': f1,
            'structural_hamming_distance': self.structural_hamming_distance / len(self.computation_times) if self.computation_times else 0.0,
...
class MultiModalMetrics:
    ...
    def update(self, modality_features: Dict[str, np.ndarray], 
        ...
        if len(modality_features) >= 2:
            ...
            corr = np.corrcoef(modality_features[modalities[0]].flatten(),
                              modality_features[modalities[1]].flatten())[0, 1]
    ...
    def get_metrics(self) -> Dict[str, float]:
        """Get computed metrics"""
        metrics = {
            'avg_fusion_accuracy': np.mean(self.fusion_accuracy) if self.fusion_accuracy else 0.0,
            'avg_cross_modal_correlation': np.mean(self.cross_modal_correlation) if self.cross_modal_correlation else 0.0,
            'avg_missing_modality_robustness': np.mean(self.missing_modality_robustness) if self.missing_modality_robustness else 0.0
        }
        
        # Modality contributions
        for modality, contributions in self.modality_contributions.items():
            metrics[f'{modality}_contribution'] = np.mean(contributions) if contributions else 0.0
...
class CausalRLMetrics:
    ...
    def update(self, episode_reward: float, steps: int, 
    ...
    def get_metrics(self) -> Dict[str, float]:
        ...
        return {
            ...
            'avg_sample_efficiency': np.mean(self.sample_efficiency) if self.sample_efficiency else 0.0,
            'avg_intervention_accuracy': np.mean(self.intervention_accuracy) if self.intervention_accuracy else 0.0,
            'avg_counterfactual_quality': np.mean(self.counterfactual_quality) if self.counterfactual_quality else 0.0,
            'avg_causal_reasoning_accuracy': np.mean(self.causal_reasoning_accuracy) if self.causal_reasoning_accuracy else 0.0
        }
...
class IntegratedMetrics:
    ...
    def update_causal(self, true_graph: nx.DiGraph, predicted_graph: nx.DiGraph, 
                     computation_time: float = 0.0):
        """Update causal discovery metrics"""
        self.causal_metrics.update(true_graph, predicted_graph, computation_time)
    
    def update_multimodal(self, modality_features: Dict[str, np.ndarray], 
                         predictions: np.ndarray, targets: np.ndarray,
                         missing_modalities: List[str] = None):
        """Update multi-modal metrics"""
        self.multimodal_metrics.update(modality_features, predictions, targets, missing_modalities)
    
    def update_rl(self, episode_reward: float, steps: int, 
                 intervention_results: Dict[str, Any] = None,
                 counterfactual_results: Dict[str, Any] = None):
        """Update RL metrics"""
        self.rl_metrics.update(episode_reward, steps, intervention_results, counterfactual_results)
