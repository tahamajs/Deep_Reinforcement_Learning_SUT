...
def compute_coordination_score(actions: List[List[int]], 
                             coordination_type: str = "consensus") -> float:
...
def compute_efficiency_score(rewards: List[float], 
                           max_possible_reward: float = 100.0,
                           min_possible_reward: float = 0.0) -> float:
    """Compute efficiency score based on reward achievement."""
    if not rewards:
        return 0.0
        
    normalized_rewards = [(r - min_possible_reward) / 
...
def compute_communication_efficiency(messages_sent: List[int],
    ...
    received_rate = np.mean(messages_received) / total_possible if total_possible > 0 else 0.0
...
def analyze_policy_convergence(metrics: MultiAgentMetrics,
                             window_size: int = 100,
    ...
    convergence_episode = len(metrics.episode_rewards) - window_size if converged else -1
    
    return {
        "converged": converged,
        "convergence_episode": convergence_episode,
        "final_variance": reward_variance,
        "improvement_trend": np.mean(recent_rewards) - np.mean(metrics.episode_rewards[:window_size])
    }


def compare_algorithm_performance(metrics_list: List[MultiAgentMetrics],
