    ...
    def adapt_to_target_domain(self, target_data, num_epochs=100):
        ...
        for epoch in range(num_epochs):
            ...
            for batch in target_data:
                ...
                # Policy loss
                log_probs = torch.log(target_output.gather(1, actions.unsqueeze(1)).squeeze() + 1e-8)
            ...
            if epoch % 20 == 0:
                print(f"Transfer Learning Epoch {epoch}: Loss = {total_loss / len(target_data):.4f}")
...
class CurriculumLearningAgent:
    ...
    def update_curriculum(self, performance):
        ...
        if len(self.level_performance) >= 50:
            ...
            if avg_performance >= threshold and self.current_level < len(self.curriculum_levels) - 1:
...
class ExplainableRLAgent:
    ...
    def __init__(self, state_dim, action_dim, lr=3e-4):
        ...
        self.attention = nn.MultiheadAttention(state_dim, num_heads=8, batch_first=True)
    ...
    def get_action_with_explanation(self, state):
        ...
        with torch.no_grad():
            ...
            # Self-attention
            attended_state, attention_weights = self.attention(state_tensor, state_tensor, state_tensor)
            
            # Store attention weights for explanation
            self.attention_weights_history.append(attention_weights.squeeze().cpu().numpy())
            ...
            return action.item(), attention_weights.squeeze().cpu().numpy(), value.item()
    ...
    def update(self, trajectories):
        ...
        for trajectory in trajectories:
            ...
            states_tensor = torch.FloatTensor(states).unsqueeze(0)  # [1, seq_len, state_dim]
            actions_tensor = torch.LongTensor(actions)
            
            # Attention
            attended_states, _ = self.attention(states_tensor, states_tensor, states_tensor)
            ...
            policy_outputs = torch.stack(policy_outputs, dim=1).squeeze(0)  # [seq_len, action_dim]
            log_probs = torch.log(policy_outputs.gather(1, actions_tensor.unsqueeze(1)).squeeze() + 1e-8)
            policy_loss = -(log_probs * rewards).mean()
...
class AdaptiveMetaLearningAgent:
    ...
    def meta_update(self, tasks_data):
        ...
        for task_id, (support_data, query_data) in tasks_data.items():
            ...
            query_log_probs = torch.log(query_action_probs.gather(1, query_actions_tensor.unsqueeze(1)).squeeze() + 1e-8)
            ...
            for env_name, env_info in experiment['environments'].items():
                ...
                for episode in range(num_episodes):
                    ...
                    if episode % 100 == 0:
                        ...
                        print(f"Agent: {agent_name}, Env: {env_name}, Episode: {episode}, Avg Reward: {avg_reward:.3f}")
