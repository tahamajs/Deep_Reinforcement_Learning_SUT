...
class DynamicMultiObjectiveEnvironment:
    ...
    def __init__(self, config: EnvironmentConfig):
        ...
        # Observation space
        self.observation_space = 2 + 2 + self.num_objectives * 2 + 10  # pos + vel + objectives + local info
    ...
    def _update_objectives(self):
        ...
        if self.dynamic_changes and self.step_count % self.objective_change_frequency == 0:
    ...
    def _apply_physics(self, action):
        ...
        if velocity_magnitude > self.max_velocity:
            self.agent_velocity = self.agent_velocity / velocity_magnitude * self.max_velocity
...
class PartiallyObservableEnvironment:
    ...
    def _generate_enemies(self):
        ...
        for _ in range(3):
            ...
            enemies.append({'pos': pos, 'speed': speed, 'direction': np.random.uniform(0, 2*np.pi)})
        return enemies
    ...
    def _get_visible_objects(self):
        ...
        for target in self.targets:
            if not target['collected']:
                ...
                if distance <= self.visibility_radius:
                    ...
                    angle_to_target = np.arctan2(target['pos'][1] - self.agent_pos[1], 
        ...
        for obstacle in self.obstacles:
            distance = np.linalg.norm(self.agent_pos - obstacle)
            if distance <= self.visibility_radius:
                angle_to_obstacle = np.arctan2(obstacle[1] - self.agent_pos[1], 
                                             obstacle[0] - self.agent_pos[0])
        ...
        for enemy in self.enemies:
            ...
            if distance <= self.visibility_radius:
                angle_to_enemy = np.arctan2(enemy['pos'][1] - self.agent_pos[1], 
    ...
    def _update_enemies(self):
        ...
        for enemy in self.enemies:
            ...
            enemy['pos'] += enemy['speed'] * np.array([np.cos(enemy['direction']), 
                                                     np.sin(enemy['direction'])])
    ...
    def step(self, action):
        ...
        # Apply action
        if action == 0:  # forward
            new_pos = self.agent_pos + 0.5 * np.array([np.cos(self.agent_orientation), 
                                                     np.sin(self.agent_orientation)])
            if self._is_valid_position(new_pos):
                self.agent_pos = new_pos
        elif action == 1:  # left
            new_pos = self.agent_pos + 0.5 * np.array([np.cos(self.agent_orientation + np.pi/2), 
...
class ContinuousControlEnvironment:
    ...
    def __init__(self, config: EnvironmentConfig):
        ...
        self.observation_space = 8  # pos, vel, angle, angular_vel, target_rel_pos, obstacle_dist
    ...
    def get_observation(self):
        ...
        # Target information
        if self.targets:
            nearest_target = min(self.targets, key=lambda t: np.linalg.norm(self.agent_pos - t['pos']))
        ...
        if self.obstacles:
            nearest_obstacle = min(self.obstacles, key=lambda o: np.linalg.norm(self.agent_pos - o['pos']))
            distance = np.linalg.norm(self.agent_pos - nearest_obstacle['pos']) / self.size
            obs.append(distance)
        else:
            obs.append(1.0)
    ...
    def step(self, action):
        ...
        if self.targets:
            nearest_target = min(self.targets, key=lambda t: np.linalg.norm(self.agent_pos - t['pos']))
            distance_reward = -np.linalg.norm(self.agent_pos - nearest_target['pos']) / self.size
            reward += distance_reward * 0.1
        
        # Check termination
        done = all(target['collected'] for target in self.targets) or collision_penalty > 0
...
class AdversarialEnvironment:
    ...
    def __init__(self, config: EnvironmentConfig):
        ...
        self.observation_space = 2 + 2 + 2 + 3  # agent_pos + opponent_pos + target_pos + strategy_info
        self.action_space = 4
    ...
    def _get_opponent_action(self):
        ...
        if self.opponent_strategy == 'aggressive':
            ...
            if self.targets:
                nearest_target = min(self.targets, key=lambda t: np.linalg.norm(self.opponent_pos - t['pos']))
    ...
    def step(self, action):
        ...
        info = {
            'targets_collected': sum(1 for t in self.targets if t['collector'] == 'agent'),
            'opponent_strategy': self.opponent_strategy,
            'opponent_targets': sum(1 for t in self.targets if t['collector'] == 'opponent')
        }
