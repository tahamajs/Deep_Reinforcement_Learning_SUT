...
class HyperparameterTuner:
            ...
            for lr in lr_range:
                ...
                for seed in range(num_seeds):
                    ...
                    if alg_name == 'REINFORCE':
                        ...
                        agent = agent_class(self.state_dim, self.action_dim, lr_actor=lr, lr_critic=lr)
                    else:  # PPO
                        agent = agent_class(self.state_dim, self.action_dim, lr=lr)
    ...
    def _plot_lr_tuning_results(self, results: Dict[str, Any], lr_range: List[float]):
        ...
        for i, (alg_name, alg_results) in enumerate(results.items()):
            ...
            ax.errorbar(lrs, performances, yerr=stds, 
        ...
        bars = ax.bar(alg_names, best_performances, 
        ...
        for bar, lr in zip(bars, best_lrs):
            ...
            ax.text(bar.get_x() + bar.get_width()/2., height + 5,
                   f'LR={lr:.0e}', ha='center', va='bottom')
        ...
        for alg_name, alg_results in results.items():
            ...
            print(f"  Best Performance: {best_result['performance']:.2f} ± {best_result['std']:.2f}")
...
class PolicyGradientBenchmark:
            ...
            for alg_name, agent_class in self.algorithms.items():
                ...
                for seed in range(num_seeds):
                    ...
                    if alg_name == 'REINFORCE':
                        ...
                        agent = agent_class(state_dim, action_dim, baseline_type='value_function')
                    elif alg_name == 'Actor-Critic':
                        agent = agent_class(state_dim, action_dim)
                    elif alg_name == 'A2C':
                    ...
                    stability = np.std(episode_rewards[-50:]) if len(episode_rewards) >= 50 else 0
                ...
                env_results[alg_name] = {
                    'mean_final_performance': np.mean([r['final_performance'] for r in seed_results]),
                    'std_final_performance': np.std([r['final_performance'] for r in seed_results]),
                    'mean_convergence_episode': np.mean([r['convergence_episode'] for r in seed_results]),
    ...
    def _find_convergence_episode(self, episode_rewards: List[float], 
    ...
    def _plot_benchmark_results(self, results: Dict[str, Any]):
        ...
        for i, env_name in enumerate(env_names):
            ...
            performances = [env_results[alg]['mean_final_performance'] for alg in alg_names]
            stds = [env_results[alg]['std_final_performance'] for alg in alg_names]
            
            ax.bar(x + i*width, performances, width, 
                  label=env_name, yerr=stds, capsize=5, alpha=0.7)
        ...
        for i, env_name in enumerate(env_names):
            env_results = results[env_name]
            convergence_episodes = [env_results[alg]['mean_convergence_episode'] for alg in alg_names]
            
            ax.plot(alg_names, convergence_episodes, 'o-', 
                   label=env_name, linewidth=2, markersize=8)
        ...
        for i, env_name in enumerate(env_names):
            ...
            ax.bar([f"{alg}\n({env_name})" for alg in alg_names], stabilities, 
        ...
        for env_name in env_names:
            ...
            for i, alg in enumerate(alg_names):
                ax.annotate(alg, (stabilities[i], performances[i]), 
        ...
        table = ax.table(cellText=summary_data,
                        colLabels=['Environment', 'Algorithm', 'Performance', 'Convergence', 'Stability'],
                        cellLoc='center',
                        loc='center')
        ...
        for env_name in env_names:
            ...
            best_performance = max(alg_names, key=lambda x: env_results[x]['mean_final_performance'])
            best_efficiency = min(alg_names, key=lambda x: env_results[x]['mean_convergence_episode'])
            best_stability = min(alg_names, key=lambda x: env_results[x]['mean_stability'])
            
            print(f"  Best Performance: {best_performance} ({env_results[best_performance]['mean_final_performance']:.1f})")
            print(f"  Best Efficiency: {best_efficiency} ({env_results[best_efficiency]['mean_convergence_episode']:.0f} episodes)")
            print(f"  Best Stability: {best_stability} ({env_results[best_stability]['mean_stability']:.1f})")
