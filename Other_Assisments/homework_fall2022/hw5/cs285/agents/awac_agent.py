...
class AWACAgent(DQNAgent):
    ...
    def get_qvals(self, critic, obs, action):
        ...
        return qa_vals[np.arange(len(obs)), action]
    ...
    def estimate_advantage(
        ...
        if self.agent_params["discrete"]:
            qa_vals = self.exploitation_critic.qa_values(ob_no)
            v_pi = np.mean(qa_vals, axis=1)
        else:
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (
            ...
            exploitation_critic_loss = self.exploitation_critic.update(
                ob_no, ac_na, env_reward, next_ob_no, terminal_n
            )
