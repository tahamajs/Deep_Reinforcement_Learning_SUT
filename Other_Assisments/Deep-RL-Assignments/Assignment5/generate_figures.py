...
def generate_convergence_plot():
    ...
    for epoch in epochs:
        if epoch == 0:
            ...
            rate = base_rate + (final_rate - base_rate) * (1 / (1 + np.exp(-6 * (progress - 0.3))))
            # Add some noise
    ...
    plt.axhline(y=base_rate, color='r', linestyle='--', alpha=0.7, label='Random Baseline')
    plt.axhline(y=final_rate, color='g', linestyle='--', alpha=0.7, label='Ground Truth')
