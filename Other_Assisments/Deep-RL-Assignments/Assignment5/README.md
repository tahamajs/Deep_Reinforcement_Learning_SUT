...
\[
\pi^\*(s*t) = \arg\max*{\pi} \mathbb{E}\_{\tau \sim p(\tau|s_t,\pi)} [R(\tau)]
\]
...
\[
\hat{s}_{t+1} = \frac{1}{K} \sum_{i=1}^K f*i(s_t, a_t), \quad
\sigma^2(s_t, a_t) = \frac{1}{K-1} \sum*{i=1}^K (f*i(s_t, a_t) - \hat{s}*{t+1})^2
\]
...
\[
C(s) = w*p \cdot \max(d_p - r_c, 0) + w_g \cdot d_g + w_d \cdot |d*{coord}|
\]
