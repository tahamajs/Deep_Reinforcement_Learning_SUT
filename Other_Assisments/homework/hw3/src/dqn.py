...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('dqn_debug.log')
    ]
)
...
def build_q_network(input_shape, num_actions, scope, reuse=False):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        logger.info(f"🏗️  Building Q-network: scope={scope}, input_shape={input_shape}, "
        ...
        if len(input_shape) == 3:
            ...
            logger.debug(f"  📐 Conv1: filters=32, kernel=8x8, stride=4, output_shape={conv1.shape}")
            
            conv2 = _conv2d_layer(
                conv1, 64, 4, 2, activation=tf.nn.relu, name="conv2"
            )
            logger.debug(f"  📐 Conv2: filters=64, kernel=4x4, stride=2, output_shape={conv2.shape}")
            
            conv3 = _conv2d_layer(
                conv2, 64, 3, 1, activation=tf.nn.relu, name="conv3"
            )
            logger.debug(f"  📐 Conv3: filters=64, kernel=3x3, stride=1, output_shape={conv3.shape}")
            ...
            logger.debug(f"  📐 Q-values: units={num_actions}, output_shape={q_values.shape}")
        else:
        ...
        logger.info(f"✅ Q-network built successfully: {len(network_vars)} variables in scope '{scope}'")
...
class DQNAgent:
    ...
    def __init__(
        ...
        logger.info(f"  🎮 Environment: {env.spec.id if hasattr(env, 'spec') else 'Unknown'}")
        logger.info(f"  📊 Input shape: {input_shape}, Actions: {self.num_actions}")
        logger.info(f"  🔧 Config: batch_size={batch_size}, gamma={gamma}, learning_starts={learning_starts}")
        logger.info(f"  📚 Replay buffer: size={replay_buffer_size}, learning_freq={learning_freq}")
        logger.info(f"  🎯 Target update: freq={target_update_freq}, double_q={double_q}")
    ...
    def update_model(self):
        ...
        if (
            ...
            logger.debug(f"🔄 Training step: t={self.t}, buffer_size={len(self.replay_buffer)}")
            
            obs_t_batch, act_t_batch, rew_t_batch, obs_tp1_batch, done_mask_batch = (
                self.replay_buffer.sample(self.batch_size)
            )
            
            logger.debug(f"  📊 Batch stats: rewards_mean={np.mean(rew_t_batch):.3f}, "
            ...
            logger.debug(f"  📉 Loss: {loss_before:.6f} -> {loss_after:.6f} "
                        f"(change: {loss_after-loss_before:+.6f})")
            
            if self.num_param_updates % self.target_update_freq == 0:
                logger.info(f"🎯 Updating target network (update #{self.num_param_updates})")
                self.sess.run(self.update_target_fn)

            self.num_param_updates += 1
            
            if self.num_param_updates % 100 == 0:
                logger.info(f"📊 Training progress: updates={self.num_param_updates}, "
