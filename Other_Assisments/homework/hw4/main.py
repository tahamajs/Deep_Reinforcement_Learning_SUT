...
parser.add_argument('--env', type=str, default='HalfCheetah', choices=('HalfCheetah', 'Pendulum'))
...
parser.add_argument('--overwrite', action='store_true',
                    help='Overwrite existing experiment directory if it already exists.')
...
# Import modules AFTER the MuJoCo check
from logger import logger
from model_based_rl import ModelBasedRL

# Import environment modules based on selection
...
if os.path.exists(exp_dir):
    if args.overwrite:
        print(f"⚠️  Experiment directory '{exp_dir}' exists. Removing because --overwrite was set.")
        shutil.rmtree(exp_dir)
    else:
        raise FileExistsError(
            "Experiment directory {0} already exists. Either delete the directory, "
            "use --overwrite, or run the experiment with a different name.".format(exp_dir)
        )
