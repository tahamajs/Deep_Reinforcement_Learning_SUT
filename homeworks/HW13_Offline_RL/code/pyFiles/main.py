...
def train_and_evaluate(algorithm_name, agent, dataset, env, num_episodes=100):
    ...
    actions = torch.tensor(dataset["actions"], dtype=torch.float32).unsqueeze(1)  # [batch, 1]
    rewards = torch.tensor(dataset["rewards"], dtype=torch.float32)
    next_states = torch.tensor(dataset["next_states"], dtype=torch.float32)
    ...
    for epoch in range(num_episodes):
        ...
        if algorithm_name == "BCQ":
            ...
            if "q_loss" in losses:
                train_losses["q_loss"].append(losses["q_loss"])
            if "policy_loss" in losses:
                train_losses["policy_loss"].append(losses["policy_loss"])
    ...
    if algorithm_name == "BCQ":
        ...
        # For continuous action spaces (other algorithms)
        def policy_fn(state):
            state_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0)
            action = agent.select_action(state_tensor).detach().numpy()[0]
            return action
...
def main():
    ...
    # Train dynamics model (simplified for demonstration)
