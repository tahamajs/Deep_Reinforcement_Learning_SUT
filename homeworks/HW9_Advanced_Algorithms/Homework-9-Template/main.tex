...
\textbf{Trust Region Idea:}

Only update policy within a region where we "trust" our estimates.

\begin{equation}
\text{Trust region: } \{\theta : \text{KL}(\pi_{\theta_{\text{old}}} \| \pi_\theta) \leq \delta\}
\end{equation}
...
\node at (0,0) {π_old};
\node at (1,1) {π_1 (good)};
\node at (-1,-1) {π_2 (boundary)};
\node at (2.5,2.5) {π_3 (collapsed)};
\draw[dashed] (0,0) -- (1,1);
\draw[dashed] (0,0) -- (-1,-1);
\draw[dashed] (0,0) -- (2.5,2.5);
\node at (0,-2.5) {Trust region (circle of radius δ in KL)};
\node at (1.2,1.2) {Safe update};
\node at (-1.2,-1.2) {Maximal safe update};
\node at (2.7,2.7) {Potentially catastrophic};
\end{tikzpicture}
\end{center}
