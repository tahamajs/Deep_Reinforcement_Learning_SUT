...
The dueling network architecture addresses a fundamental limitation in standard DQN: the inability to separate state value from action advantage. This separation provides better generalization and more efficient learning.

\textbf{Standard DQN Limitation:}

Standard DQN directly estimates Q-values:
\begin{equation}
Q(s,a) = f_\theta(s,a)
\end{equation}

This approach conflates two distinct concepts:
\begin{itemize}
\item \textbf{State Value}: How good is state $s$?
\item \textbf{Action Advantage}: How much better is action $a$ compared to average?
\end{itemize}

\textbf{Dueling Architecture:}

The dueling network decomposes Q-values into value and advantage components:
...
where:
\begin{itemize}
\item $V(s)$: State value function
\item $A(s,a)$: Action advantage function
\item $\bar{A}(s) = \frac{1}{|\mathcal{A}|}\sum_{a'} A(s,a')$: Mean advantage (for identifiability)
\end{itemize}
...
\textbf{1. Backward Computation:}
\begin{itemize}
\item Retrace targets computed backward in time
\item Each target depends on future targets
\item Efficient vectorized implementation
\end{itemize}

\textbf{2. Importance Sampling:}
\begin{itemize}
\item Compute $\rho_t = \frac{\pi(a_t|s_t)}{\mu(a_t|s_t)}$
\item Truncate with $\min(1, \rho_t)$
\item Add small epsilon for numerical stability
\end{itemize}

\textbf{3. Trace Coefficients:}
\begin{itemize}
\item $c_t = \lambda \cdot \min(1, \rho_t)$
\item Bounded by $\lambda$ for stability
\item Controls bias-variance trade-off
\end{itemize}

\textbf{Benefits:}
\begin{itemize}
\item \textbf{Low Variance}: Truncated IS weights prevent explosion
\item \textbf{Safe Off-Policy}: Works with any behavior policy
\item \textbf{Multi-Step}: Fast reward propagation
\item \textbf{Sample Efficient}: Better than standard IS
\end{itemize}
