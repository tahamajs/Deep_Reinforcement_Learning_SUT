...
class GaussianPolicy(nn.Module):
    ...
    def __init__(self, num_inputs: int, num_actions: int, hidden_dim: int, action_space) -> None:
    ...
    def sample(self, state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        y_t = torch.tanh(x_t)
        action = y_t * self.action_scale.to(x_t.device) + self.action_bias.to(x_t.device)
        # log_prob before and after tanh
        log_prob = normal.log_prob(x_t)
        # correction for Tanh squashing
        log_prob -= torch.log(self.action_scale.to(x_t.device) * (1 - y_t.pow(2)) + epsilon)
        log_prob = log_prob.sum(1, keepdim=True)
        mean_action = torch.tanh(mean) * self.action_scale.to(mean.device) + self.action_bias.to(mean.device)
        return action, log_prob, mean_action


class DeterministicPolicy(nn.Module):
    """Deterministic policy for DDPG-style algorithms (with noise for exploration)."""
    def __init__(self, num_inputs: int, num_actions: int, hidden_dim: int, action_space=None) -> None:
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        x = F.relu(self.linear1(state))
        x = F.relu(self.linear2(x))
        mean = torch.tanh(self.mean(x)) * self.action_scale.to(x.device) + self.action_bias.to(x.device)
        return mean

    def sample(self, state: torch.Tensor, noise_scale: float = 0.1) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
