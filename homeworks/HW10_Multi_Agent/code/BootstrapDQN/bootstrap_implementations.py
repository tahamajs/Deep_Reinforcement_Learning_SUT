"""
Complete implementations for BootstrapDQN homework
Copy these implementations into the notebook cells
"""
...
class MultiHeadQNet(nn.Module):
    ...
    def forward(self, x, head_idx=None):
        ...
        if head_idx is not None:
            ...
            return torch.stack(
                outputs, dim=1
            )  # Shape: (batch_size, num_heads, output_dim)


# ============================================================================
# BootstrapDQNAgent Implementation
# ============================================================================
...
class BootstrapDQNAgent(EpsGreedyDQNAgent):
    ...
    def _compute_loss(self, batch):
        ...
        actions_expanded = (
            actions.unsqueeze(1).unsqueeze(2).expand(batch_size, self.k, 1)
        )
        q_values = q_values_all_heads.gather(2, actions_expanded).squeeze(
            2
        )  # (batch_size, k)
...
# ============================================================================
# PriorMultiHeadQNet Implementation
# ============================================================================
...
class PriorMultiHeadQNet(MultiHeadQNet):
    ...
    def __init__(
        self, input_dim, output_dim, num_heads=10, hidden_dim=256, prior_hidden_dim=128
    ):
...
# ============================================================================
# RPFBootstrapDQNAgent Implementation
# ============================================================================
...
class RPFBootstrapDQNAgent(BootstrapDQNAgent):
    ...
    def _act_in_training(self, state):
        ...
        if torch.rand(1).item() < self.epsilon:
            ...
            with torch.no_grad():
                ...
                trainable_q, prior_q = self.q_network(
    ...
    def _compute_loss(self, batch):
        ...
        with torch.no_grad():
            next_trainable_q, next_prior_q = self.target_network(
                next_states, return_prior=True
            )
...
# ============================================================================
# UEBootstrapDQNAgent Implementation
# ============================================================================
    ...
    def _wandb_train_step_dict(self):
        ...
        if hasattr(self, "replay_buffer") and len(self.replay_buffer) > 0:
            ...
            try:
                ...
                with torch.no_grad():
                    trainable_q, prior_q = self.q_network(
                        sample_batch["state"], return_prior=True
                    )
                    combined_q = trainable_q + self.beta * prior_q
...
print("\nCopy each class implementation into the corresponding notebook cell.")
print("\nMake sure to:")
print("1. Copy MultiHeadQNet into the first implementation cell")
print("2. Copy BootstrapDQNAgent into the second implementation cell")
print(
    "3. Copy PriorMultiHeadQNet and RPFBootstrapDQNAgent into the third implementation cell"
)
print("4. Copy UEBootstrapDQNAgent into the fourth implementation cell")
